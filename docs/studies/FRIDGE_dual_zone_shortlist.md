...
- Dual-zone (separate temp control for fridge + freezer use)
- Capacity in the `30-48qt` range
- Budget under `$750`
- Does **not** require an integrated rechargeable battery
- Suitable for semi-permanent install on a fridge slide (not intended to be frequently removed)

## Important limitation in this pass

This environment could not reach external retail/manufacturer sites (HTTP proxy returned `403` on outbound requests), so exact live prices and in-stock status could not be re-validated during this pass.

Because of that, the list below is a practical **starting shortlist** based on commonly available overland/12V models and typical market pricing bands, not a live-price guarantee.

## Strong-fit shortlist (likely to match your constraints)

| Model | Claimed class capacity | Typical street-price band (USD) | Why it fits your ask | Caveats to verify before purchase |
| --- | --- | ---: | --- | --- |
| ICECO `VL45ProD` | about `47.5qt` | `$650-$750` | Dual zone, right at top end of your budget, proven slide-friendly form factor | Verify actual sale price at checkout; confirm external dimensions vs slide width |
| BODEGACOOLER `TWW45` dual-zone | about `45qt` | `$300-$450` | Usually one of the most budget-friendly dual-zone options in your size target | Verify compressor warranty/support quality and latch robustness |
...
Before ordering, verify these with exact model dimensions from the product page/spec PDF:

- **Overall width + handle protrusion** fits your slide tray with `>=0.5 in` side clearance.
- **Lid swing clearance** (many fridges need rear or top clearance to fully open).
- **Ventilation gap** around compressor side: target `>=1.5 in` at compressor intake/exhaust side.
- **Tie-down points** align with slide anchors (or add an adapter plate).
- **12V wiring path** supports full extension without rubbing; use strain relief near the fridge socket.
- **Low-voltage cutoff setting** can be adjusted for your house electrical strategy.

## Recommended next move (quick validation workflow)

Do this before buying anything:

1. Pick top `2` models from the shortlist.
2. Pull exact `L x W x H` + lid-open envelope from official spec sheets.
3. Cross-check against your intended fridge-slide tray dimensions.
4. Compare real checkout totals (unit + shipping + tax + warranty).
5. Buy whichever lands under `$750` with the best warranty and dimensional fit.

## If you want, I can do a live buy-list pass next

If outbound web access is available in your next run, I can produce:

- current live prices from `3-5` retailers per model
- a final ranked pick with direct buy links
- a slide-fit go/no-go table using your exact available footprint
