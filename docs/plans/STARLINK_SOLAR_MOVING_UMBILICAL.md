...
Last current-source check: `2026-08-06 05:54 MDT`
...
Mechanical fallback remains two movable black `10-series 1020` (`1 x 2 in`) extrusion crossbars carried by the existing longitudinal roof tracks. For Yakima-style tracks, the dimensional lead is one set of eight Yakima `8810074` Anchor Plate A inserts (`3/4 x 7/8 in`, centered M6 thread), inserted from a track end rather than through the approximately `1/2 in` top slot. Use the TRIO four corner points and reusable vibration-resistant hardware. Choose this path instead if the roof is curved/ridged, locally flexible, oxidized/chalky, porous, or cannot provide full flat disc contact.
...
Immediate bench/fit package is the AUDEETO pigtail pair, ZBLZGP 3 m coil, `NE8FDX-P6-W`, `NE8MXR1-B-TOP-D` after the coil OD check, and a field-measured fixed Cat6A run. Add the Furnique inline coupler only if panel-fed factory-cable ground mode is worth the extra interface. The `26 AWG` pigtails/coil are accepted only after continuity/shield inspection and the loaded heat/dropout test. Dry-fit the TRIO frame plus magnet/disc contact points and an independent tether before choosing magnets over the crossbar fallback. Do not cut the camper wall or coil or bond VHB discs until connector, wall-stack, roof-flatness, and removal mockups are physically proven.
...
| 1, optional 2nd | Neutrik `NE8MXR1-B-TOP-D` | [Mouser](https://www.mouser.com/ProductDetail/Neutrik/NE8MXR1-B-TOP-D) — `$8.93` each | One on roof coil; a second can terminate the short factory-cable panel jumper and also provides first-termination insurance. |
...
### Roof-track/extrusion package — mechanical fallback if VHB/disc contact fails
...
Current observed-price required core package is about `$430` before the measured interior cable, roof attachment, DC cable/fuse/terminations, shipping, and tax. The TRIO magnet/disc package plus normal interior/DC allowances brings the current installation subtotal to about `$555` before shipping/tax. Add about `$27` for the optional factory-cable-through-panel kit (inline coupler, second Neutrik termination, and short cable), for about `$582` before shipping/tax. Allow roughly `$560-600` installed without that optional mode or `$590-620` with it. Nothing in this section is marked purchased until owner confirmation or a purchase record is reconciled.
