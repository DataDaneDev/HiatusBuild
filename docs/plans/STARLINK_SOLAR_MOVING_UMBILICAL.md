...
Status: **Starlink fixed-body pass-through/retractile and switched `12V` conversion operational; final weather/travel closeout open. Four solar panels received. PV fixed-side work is complete through the shell pass-through, two-pole disconnect, stationary wiring, and MPPT landing; no panels are connected. Roof preparation, structural-silicone bonding/cure, `4S1P` panel wiring, moving-cord support/full-motion proof, and controlled PV commissioning remain open.**
Last current-source check: `2026-09-02`
...
Solar remains separate from the Starlink penetration. All four purchased Renogy `175W` panels have arrived; one-panel physical test-fit plus measured layout indicates that the four-panel package fits with Starlink and MaxxAir while using nearly the whole roof. The electrical path remains one `4S1P` string on the Victron `150/45`. Owner report `2026-09-02`: the shell pass-through, two-pole disconnect, stationary wiring, and MPPT landing are complete, but no panels are connected. Direct roof bonding still waits on exact structural-silicone product, substrate/backsheet preparation, coupon/cure proof, drainage, and one-panel shakedown.

The selected PV path uses one continuous retractile cord in the front camper-to-cab gap; its fixed-camper compression gland/pass-through is installed just above the Starlink entry behind the cab. Keep the helix external and retain independent structural support so adhesive guides, the gland, and terminals carry no spring, wind, or branch load. During roof-side wiring, prove the moving section through full roof/cab articulation. Preserve three panel-to-panel series mates and only two free string-end leads entering the completed compact two-pole transition, continuous `12 AWG` two-conductor cord, fixed load-break, and MPPT. `12 AWG` is conductor size, not circuit voltage. There is no exterior PV quick disconnect or manual roof-motion release step.
