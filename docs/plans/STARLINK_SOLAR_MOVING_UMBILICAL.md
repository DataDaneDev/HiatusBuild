...
Last current-source check: `2026-08-25`
...
Put the panel bulkhead in rigid fixed-body structure below the folding/moving seam and near enough to the Desk/router that the interior patch cable remains serviceable. Put the upper structural anchor directly above it on moving-roof structure/rail. Add a P-clamp behind the upper adapter joint and strain relief at the panel so neither electrical connector carries spring tension. Both Neutrik cable assemblies are now physically complete. The owner reports that the green retractile cable was a very tight fit in the Neutrik termination and required lubricant and substantial insertion force, but a DMM pin-by-pin check showed continuity on every conductor at both finished plug ends. That closes conductor continuity only; shield-shell continuity, loaded Starlink heat/dropout/reboot behavior, full-roof-stroke strain relief, and spray/retention checks remain open. The exact panel location, wall stack, anchor spacing, interior cable length, free extension, tangent lengths, and down-state coil projection still require a physical mockup.
...
| Fixed-body RJ45 bulkhead | [Neutrik `NE8FDX-P6-W`](https://www.neutrik.com/en/product/ne8fdx-p6-w) / [Mouser](https://www.mouser.com/ProductDetail/Neutrik/NE8FDX-P6-W) | **Received; location/cut pending.** One purchased at `$28.45`. Shielded Cat6A feedthrough, PoE Type 4 Class 8 / `100W`, `>1000` cycles, rugged latch, captive cap, and IP65 correctly mated/capped. Rear panel plug is ordinary RJ45. Published rear-side body depth is about `29.5 mm`; allow additional cable bend/service space. |
...
Orders placed `2026-08-06`: Mouser one `NE8FDX-P6-W` plus two `NE8MXR1-B-TOP-D`, `$46.31` item subtotal / `$54.80` checkout total; TRIO white Speedmount, `75 mm` through-hardware, magnets, and VHB-backed discs, `$365` with free shipping; Amazon pigtail pair, Furnique coupler, ZBLZGP coil, 10 ft and 1 ft Cable Matters patch cables, EAZUSE converter, and VCELINK crimper, `$141.44` combined item subtotal. By `2026-08-12`, both Neutrik cable assemblies were complete and every green-coil conductor passed the owner's DMM continuity check. The same DMM method can replace the separate basic-tester purchase, but repeat it on the short jumper; shield-shell continuity and the loaded heat/dropout test remain open. Dry-fit the magnet/disc points and the optional direct-through-bolted crossbar package before committing the roof route. Do not cut the camper wall or bond VHB discs until connector, wall-stack, roof-flatness, and removal mockups are physically proven.
...
**Current penetration gate:** cable-side parts are installed and conductor continuity passed. Do not drill until the final Starlink position is confirmed and a printed/cardboard cutout proves exterior cap/latch access, rear connector and bend clearance, wall-stack clamping/backing, interior service access, drip loop, and the entire roof stroke. The official Neutrik rear-side cutout is one `>=24 mm` center opening plus four `>=3.2 mm` M3 clearance holes on `19.0 +/- 0.1 mm` horizontal by `24.0 +/- 0.1 mm` vertical centers. A `15/16 in` hole saw is `23.8125 mm` and is therefore undersize; a nominal `1 in` hole saw is the sensible center-opening tool, but drill a scrap coupon first and verify the actual connector body/gasket fit. Use a `#30` (`3.264 mm`) bit for the four fastener holes when available, or `9/64 in` (`3.572 mm`) after checking flange coverage; `1/8 in` (`3.175 mm`) is nominally undersize. The center hole alone does not retain the connector: use all four fasteners and preserve even gasket compression.
...
| 1 | Neutrik `NE8FDX-P6-W` panel feedthrough | **Received; mounting pending** — purchased from Mouser 2026-08-06 at `$28.45` | Fixed-body female/female feedthrough with captive cap. Final position, wall stack, and cutout remain gated. |
...
No blind Amazon buy cleared this use. The revised `28 in` stroke makes the **short `1-5 ft` stock class** the correct research lane; `2-10 ft` is no longer the default. Buy only after the exact part's drawing proves its total relaxed length fits the roof-down gap and its approved working reach clears the measured roof-up span with reserve. If stock sellers cannot provide that drawing and DC/environmental approval, use the Philatron/Autac exact-fit RFQ instead of accepting a long coil.
