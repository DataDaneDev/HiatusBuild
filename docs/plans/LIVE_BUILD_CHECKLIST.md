...
- Standard 4 X routing is now a one-panel package: Router 3 in an accessible ventilated Desk/service cubby, powered primarily by the selected dedicated-fuse `12V`-to-`57V` converter with factory AC/inverter fallback -> lower AUDEETO passive Gen 3 weather/retention pigtail -> fixed interior shielded pure-copper Cat6A -> one Neutrik `NE8FDX-P6-W` rugged shielded panel feedthrough -> `NE8MXR1-B-TOP-D`-terminated removable 3 m stretched shielded retractile roof jumper -> upper AUDEETO pigtail -> terminal. Mouser order placed `2026-08-06`, pending fulfillment: one panel feedthrough and two TOP cable connectors, `$46.31` item subtotal / `$54.80` checkout total. The `$19.99` pigtail two-pack replaces the `$58.99` STARGEAR pair but remains bench-test-required. Preferred recovery is the complete factory cable direct terminal-to-router through the existing service route. Optional factory-cable-through-panel mode uses one female/female Gen 3/Cat6-Cat7 coupler plus a short shielded panel jumper; the purchased second TOP connector supports that jumper. Current roof-track cart candidate uses eight `20 x 20 x 4 mm`-head `M6 x 65 mm` T-bolts to directly through-bolt two `1020` crossbars in the `2 in`-vertical orientation, two bolts per crossbar end; the carted `M6 x 35 mm` variant is too short and track fit remains a physical end-feed gate. Confirm two full-length crossbar blanks because the BOM records one `94 in` `1020` stick. Disconnect/remove the framed dish for tight brush because edge protection is not branch-proof. Solar is explicitly deferred and will receive its own PV-rated route only after panel/string `Voc`/`Isc` are known. See [STARLINK_SOLAR_MOVING_UMBILICAL](STARLINK_SOLAR_MOVING_UMBILICAL.md).
