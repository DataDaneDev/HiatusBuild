...
- Plumbing is active bench/mockup work. The new `60 mm / 2-3/8 in` sender opening is now cut and the first KUS/`FLS-U` installation was attempted, but one screw/backing-ring thread stripped or galled and left a seized screw with no clamp load; an inverted water test poured through the sender gasket. Hold tank installation/use until the failed hardware is removed, the `FLS-U` ring/main gasket/matched screws are replaced, and a dry-area leak test passes. Fill/vent and camper service penetrations remain gated. Build cold-first and hot-ready as a compact fixed pack with minimum fittings; removal of the cooler/panels/part of the 80/20 frame is acceptable for uncommon service.
