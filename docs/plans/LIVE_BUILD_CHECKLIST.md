...
- Standard 4 X routing is now a one-panel package: Router 3 in an accessible ventilated Desk/service cubby, powered primarily by the selected dedicated-fuse `12V`-to-`57V` converter with factory AC/inverter fallback -> lower AUDEETO passive Gen 3 weather/retention pigtail -> fixed interior shielded pure-copper Cat6A -> one Neutrik `NE8FDX-P6-W` rugged shielded panel feedthrough -> `NE8MXR1-B-TOP-D`-terminated removable 3 m stretched shielded retractile roof jumper -> upper AUDEETO pigtail -> terminal. The `$19.99` two-pack replaces the `$58.99` STARGEAR pair but remains bench-test-required. Preferred recovery is the complete factory cable direct terminal-to-router through the existing service route. Optional factory-cable-through-panel mode uses one female/female Gen 3/Cat6-Cat7 coupler plus a short shielded panel jumper; a third device pigtail is not the right part because both it and the factory cable end are male. Current removal-first mount lead is the TRIO Speedmount with TRIO's four rubber-coated magnets and four `3M 5952 VHB` steel discs on proven flat/sound fiberglass, about `72 h` cure, low-speed reinspection, and an independent Yakima-track tether; use mechanical crossbars if roof contact or gelcoat quality fails. Disconnect/remove the framed dish for tight brush because edge protection is not branch-proof. Solar is explicitly deferred and will receive its own PV-rated route only after panel/string `Voc`/`Isc` are known. See [STARLINK_SOLAR_MOVING_UMBILICAL](STARLINK_SOLAR_MOVING_UMBILICAL.md).
