...
- [ ] Rough in `C-21` ICECO with `12 AWG` duplex / `15A`, `C-23` SHURflo with `14 AWG` duplex / `10A`, and both `C-34` office PD and `C-35` Galley PD with `12 AWG` duplex / `15A` each. Leave the source fuse removed from any branch whose load end is not yet terminated.
