...
- Solar remains a discovery/provisioning workstream, while the Standard 4 X is in hand. The two existing longitudinal roof tracks already serve as the outer rails; use movable extrusion crossbars rather than duplicating a full perimeter. Current cable-route default is one shared *physical* roof umbilical at the sheltered front/high side: preserve the continuous factory Starlink cable and carry it beside a separately rated continuous-flex PV pair in an energy chain or controlled straight-loop guard. A single fixed-body multi-cable seal/service pod may combine the wall entry, but the Starlink and PV circuits retain separate cables, terminations, isolation, and tests. No roof penetration, sliding gland, RJ45 coil purchase, or final PV wire/connector buy is released until down/up geometry and the actual array `Voc`/`Isc`/stringing are measured.
...
5. **Prototype the combined physical Starlink/PV umbilical without a roof penetration:** mock one protected front/high-side energy chain or controlled loop carrying the unmodified factory Starlink cable plus a separate PV-cable surrogate. Prove roof-down, intermediate, and roof-up geometry plus a quick-pin trail-stow position before buying the carrier or cutting the fixed sidewall. Treat an RJ45 Starlink coil as a last fallback because Standard 4 X guidance discourages modified/combined cable paths.
