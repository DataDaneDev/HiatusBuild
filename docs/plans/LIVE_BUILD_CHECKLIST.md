...
7. **Keep Starlink/solar off the fabrication critical path:** use ground-deployed Starlink through the existing truck-bed service route for the first trip if needed. Prototype the Type-4/RJ45 adapter plus coil under load and measure roof travel/sidewall geometry before any exterior hole. The exact solar jumper remains array-gated; the current rotary service-disconnect lead is the IMO `SI32-PEL64R-2`, but final release still waits for cold `Voc`/`Isc` and stringing.
...
- [ ] Build measured removable bug screens for the rear barn-door opening and passenger-side lift hatch. Owner-confirmed `2026-08-05` inside fixed-opening measurements are rear `55.5 in W x 44.25 in H` and passenger-side `57.5 in W x 19.75 in H`. Working finished-screen envelopes are rear `59.5 in W x 46.25 in H` and side `61.5 in W x 21.75 in H`, providing `2 in` mounting overlap at both vertical edges and the top while keeping the bottom flush. Use bound mesh, full-frame sewn hook-and-loop or adhesive-backed snap attachment only on fixed surfaces outside gasket/compression paths, a center magnetic split, and a weighted/magnetic lower hem only where its landing surface is proven. Template both openings and pass a door/hatch closure-thickness test before ordering non-returnable custom screens or applying permanent adhesive.
- [ ] Template the passenger-side screen around the paired gas-strut/bracket obstructions centered on both vertical edges; each projects about `1.5 in` into the screen plane. Prefer reinforced relief slots or small gussets with local attachment tabs over simply stretching the net around the bumps, which can pull the perimeter open and create insect gaps.
