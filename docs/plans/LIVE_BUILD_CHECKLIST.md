...
- [ ] Mock the remote fill from the angled bed-wall deck fill through the purchased `2 in` fuel-rated straight/45-degree hose and supported beaded aluminum joiners to Speedway `91676553`, preserving continuous fall and road/heat/axle clearance. Mock one upper port capped with the freed lower cap and the other as `-10AN -> 5/16 in barb -> 5/16 in NBR hose -> one upright remote rollover vent terminating outside under the bed`. Do not install two check valves in series. Leak/vent-test a small outdoor fill before service.
