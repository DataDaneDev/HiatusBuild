...
- Owner report `2026-08-03`: all three passenger-rear water penetrations are physically installed and the replacement KUS `FLS-U` under-ring, main gasket, and matched hardware are installed and look correctly seated. Final water-system acceptance still waits for sender/full-system leak proof, final fill/vent hookup, sink termination, and one dry-compartment pressure/dwell inspection after every disturbed joint is closed.
...
1. **Finish the isolated-bank sequence:** complete Battery 2 absorption and Battery 3 charging, rest/record all three, and parallel only at `<=0.1V` maximum-to-minimum rested difference.
2. **Pull the access-dependent wiring before the counter:** rough in the office/Galley `12/3` AC branches, fridge/pump/office-PD/Galley-PD DC branches, and KUS sender pair with both ends labeled. Keep incomplete branches de-energized—AC breakers off and DC fuses removed—with every loose conductor individually capped.
3. **Close the real road-restraint gaps:** build low perimeter/divider capture around the four batteries and strap each battery down to the anchored extrusion/frame; add the ICECO foot-level strap plus lateral hard stop/soft bumper. Do not use wood screws into plywood as the only dynamic load path.
4. **Build the Galley after the wire paths are protected:** clamp the plywood counter in final position, verify sink/faucet/drain/rail clearance, then cut and install the sink/faucet. Reinstall the Desk/storage modules only after their hidden wire routes are pulled.
5. **Close KUS/water acceptance:** the replacement under-ring/gasket/hardware is installed; keep it visible through the next leak proof, finish fill/vent and sink terminations, then perform the final dry-compartment pressure/dwell inspection.
6. **Prototype the removable twin-coil Starlink/PV moving section without a roof penetration:** measure actual fixed-to-moving-anchor travel, tangent/strain-relief lengths, allowable down-state coil body/projection, and connector-entry envelope. Bench-test the Type-4/RJ45 adapter plus shielded coil under live Starlink load before installing it; preserve the complete OEM cable as a recovery spare. Size the solar coil and two-pole DC PV disconnect only after panel/string cold `Voc` and `Isc` are locked. Prove roof-down, intermediate, roof-up, normal-road, and powered-down trail-removal states before final holes.
...
- [ ] Rough in `C-31` office and `C-32` Galley AC branches with `12/3` black/white/green cable and one `20A` branch breaker per circuit. Until the GFCIs are installed, keep each breaker off and individually cap all three far-end conductors inside a listed outlet box with a cover.
- [ ] Rough in `C-21` ICECO with `12 AWG` duplex / `15A`, `C-23` SHURflo with `14 AWG` duplex / `10A`, `C-34` office PD with `12 AWG` duplex / `20A`, and `C-35` Galley PD with `12 AWG` duplex / `15A`. Leave the source fuse removed from any branch whose load end is not yet terminated.
- [ ] Pull `C-48` KUS-to-Cerbo as one continuous `18-22 AWG` duplex run with no external fuse or chassis return. Label both ends and leave it unlanded if the exact Tank-input terminal block is not in hand.
