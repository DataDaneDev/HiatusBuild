...
- [x] Acquire the water-penetration cutters (`3-1/4 in` for the IZTOR camper-wall hatch and `1-1/2 in` for the selected large-boss membrane field-fit) plus supplemental `1/2 in` PEX-B `90` elbows and cinch clamps (`2026-07-27`). The tank-boss cutter must visibly clear the internal threads before use; the Rainpal service ports use `27 mm / 1-1/16 in` openings and may use the existing marked step bit.
