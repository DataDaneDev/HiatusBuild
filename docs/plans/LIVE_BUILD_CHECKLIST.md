...
- Standard 4 X routing is now a one-panel package: Router 3 in an accessible ventilated Desk/service cubby, powered primarily by the selected dedicated-fuse `12V`-to-`57V` converter with factory AC/inverter fallback -> lower AUDEETO passive Gen 3 weather/retention pigtail -> fixed interior shielded pure-copper Cat6A -> one Neutrik `NE8FDX-P6-W` rugged shielded panel feedthrough -> `NE8MXR1-B-TOP-D`-terminated removable 3 m stretched shielded retractile roof jumper -> upper AUDEETO pigtail -> terminal. Mouser order placed `2026-08-06`, pending fulfillment: one panel feedthrough and two TOP cable connectors, `$46.31` item subtotal / `$54.80` checkout total. TRIO order placed the same day, pending: white Gen 3 Standard Speedmount with `75 mm` stainless through-bolting hardware at `$285`, rubber-coated magnets at `$40`, and VHB-backed mounting discs at `$40`; `$365` total with free shipping. The magnet route remains roof-contact-gated; the supplied `75 mm` hardware preserves frame-to-extrusion hard mounting. Preferred recovery is the complete factory cable direct terminal-to-router through the existing service route. Optional factory-cable-through-panel mode uses one female/female Gen 3/Cat6-Cat7 coupler plus a short shielded panel jumper; the purchased second TOP connector supports that jumper. Current hard-mount mockup uses mostly leftover `1010` directly over the Yakima tracks plus continuous angle-aluminum crossmembers; direct track-to-track angle may omit the longitudinal extrusion. The `M6 x 35 mm`, `20 x 20 x 4 mm` track-bolt candidate is plausible for bare `1010` but must be end-feed/stack tested; Yakima `8810074` Anchor Plate A plus field-selected `M6 x 1.0` bolts is the known-fit alternative. Keep wide-span members continuous and do not use flat bar laid flat as the default beam. Disconnect/remove the framed dish for tight brush because edge protection is not branch-proof. The selected solar architecture remains electrically and mechanically separate from Starlink; do not buy or cut its route hardware until the measured PV jumper, disconnect, strain-relief, and entry gates pass.
- Solar product/topology are now selected separately: `6x BougeRV Arch Pro 100W`, `3S2P`, on the purchased Victron `150/45`. Panel purchase waits on the roof/fan/Starlink survey, `1:1` templates, cassette design, preliminary weight budget, and any required insurer/Hiatus acceptance; bonding/travel have separate post-receipt gates.
...
7. **Keep Starlink/solar off the fabrication critical path:** the direct OEM-cable setup remains the first-trip fallback. Bench-fit the passive Gen 3/RJ45 pigtail pair, optional factory-cable coupler/panel jumper, retractile jumper, Neutrik panel connector/cable carrier, and TRIO magnet/VHB-disc contact+tether geometry before any wall/cable cut or adhesive bond. Use the crossbar fallback if full flat disc contact is not proven. Solar remains separate and array-gated; the current rotary service-disconnect lead is the IMO `SI32-PEL64R-2`, but final release still waits for cold `Voc`/`Isc` and stringing.
...
- [ ] If downstream AC receptacles are added, continue the same `12 AWG` branch from the first GFCI's `LOAD` terminals in listed accessible boxes, label them `GFCI Protected`, preserve the equipment-ground path, and retest the entire chain. More receptacles do not add branch capacity; sequence induction/toaster and other high-draw Galley loads.
...
Goal: execute the selected `600W` architecture without turning it into a full secondary roof.

- [x] Lock product/topology: `6x BougeRV Arch Pro 100W SP003 = 600W`, `3S2P`, on the purchased Victron `150/45`.
- [x] Lock mounting architecture: four direct-bond modules plus one removable, ventilated, track-supported cassette carrying the other two; no full roof deck and no rigid rack.
- [x] Lock planning geometry: measured `138 x 63 in` fiberglass, centered MaxxAir, `0.625 in` track step, and optimizer-proven mirrorable layout with `1 in` panel/fan gaps and `2 in` Starlink clearance.
- [x] Lock electrical planning screen: `142.29V` worst-case `3S` Voc at `-40F`; estimated hot/tolerance `3S` Vmp is `70.91V` at `+85C`, about `9.11V` above the `56.8V + 5V` startup threshold before route drop. Final measured route-drop/hot-roof commissioning remains required because no direct Vmp coefficient is published.
...
- [ ] Cut/place `1:1` Arch Pro templates including junction boxes, `33.5 in` leads, connectors, adhesive ribs, and service loops; cycle the fan and Starlink removal.
...
- [ ] Select which side receives the mirrorable `~91 x 26 in` two-panel cassette; complete its continuous-support, track-load, highway-uplift, windward-fairing, and no-rub isolation design.
- [ ] Complete a preliminary itemized weight budget with contingency showing the package can remain `<=75 lb`; panels plus Starlink/TRIO are `38.0 lb`, leaving `37.0 lb` before cassette and all hardware.
- [ ] Confirm insurer/Hiatus acceptance if required for adhesive-bonded roof additions.
- [ ] **Purchase release:** order panels and dedicated cassette hardware only after the preceding survey/template/design/preliminary-weight gates pass.
- [ ] Verify received `SP003` labels/polarity and reopen cold Voc if connected operation below `-40F` is intended.
- [ ] Build/load-check the cassette and weigh the **actual complete** moving-roof package at `<=75 lb`.
- [ ] Lock matched branch connectors or a listed two-string combiner, PV conductor, two-pole disconnect, roof entry, strain relief, labels, and moving jumper. `F-09` is not presently planned for only two parallel strings; confirm the omission from received labels/conductors/code.
- [ ] Bond a representative coupon/test piece using the exact polyurethane product and cure method; verify adhesion and controlled removal.
- [ ] Confirm estimated hot-string voltage minus measured route drop clears the maximum configured battery voltage by Victron's `5V` startup margin; complete stationary hot-roof commissioning.
- [ ] **Installation release:** do not bond panels or accept travel until the received-label, actual-weight, cassette-load, final-route, bond-coupon, and hot-operation gates pass.
