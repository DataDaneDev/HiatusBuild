...
- Standard 4 X routing is now a one-panel package: factory-powered router in an accessible ventilated Desk/service cubby -> lower Type-4/RJ45 adapter -> fixed interior shielded Cat6A -> one Neutrik `NE8FDX-P6-W` rugged shielded panel bulkhead -> removable shielded retractile roof jumper -> upper Type-4/RJ45 adapter -> terminal. A long outdoor shielded Cat6A cord can replace the roof jumper for ground deployment; ordinary RJ45 works at the panel in dry conditions, while wet use requires the compatible rugged/weather-sealed cable carrier. Mount the dish in a TRIO Gen 3 Standard Speedmount across two movable extrusion crossbars with four reusable fasteners/captured T-nuts; disconnect/remove the framed dish for tight brush because edge protection is not branch-proof. Preserve the complete factory cable as an unmodified recovery spare. Solar is explicitly deferred and will receive its own PV-rated route only after panel/string `Voc`/`Isc` are known. See [STARLINK_SOLAR_MOVING_UMBILICAL](STARLINK_SOLAR_MOVING_UMBILICAL.md).
