...
- [ ] Before the plywood cut, remove the nearby `12V` battery and isolate its terminals; loosen/remove only enough obstructing extrusion to keep the jigsaw flat. At maximum downstroke, set/shim/shorten the clean wood scroll blade to leave about `1/8-3/16 in` of plywood over the aluminum, drill the starter hole inside the waste, and cut slowly with orbital action off.
- [ ] Break the remaining thin plywood skin inward/upward, remove the disk and EPS, and reinspect the exposed bed plus underside. Then cut the aluminum separately with a short `18-24 TPI` bi-metal/nonferrous blade, deburr/vacuum, coat the bare edge, and complete both the cabin seal and skirt-to-bed weather seal.
...
- [x] Purchase the EVIL ENERGY `10 gal` aluminum diesel tank/feed package (`2026-08-30`, BOM row `65`) and final remote `2 in` fill / `5/16 in` KINTLE vent / neoprene-isolation package (`2026-09-02`, BOM rows `364-366`). The LF Bros `10 L` plastic tank is no longer fuel storage and must remain never-fueled for graywater use.
- [ ] Inventory the delivered diesel package before fit-up: verify tank cap/gasket, both lower `-10AN` outlets/caps, both upper `-10AN` ports, sender leads/resistance, Speedway flange/gasket/bolts, every hose/joiner/clamp, final KINTLE vent kit and adapter, neoprene, and no shipping damage. The installed remote rollover valve must be the only atmospheric vent path unless another opening has equivalent rollover shutoff.
...
- [ ] Mock the remote fill from the angled bed-wall deck fill through the purchased `2 in` fuel-rated straight/45-degree hose and supported beaded aluminum joiners to Speedway `91676553`, preserving continuous fall and road/heat/axle clearance. Mock one upper port capped with the freed lower cap and the other as `-10AN -> 5/16 in barb -> KINTLE hose -> upright remote rollover vent terminating outside under the bed`. Leak/vent-test a small outdoor fill before service.
