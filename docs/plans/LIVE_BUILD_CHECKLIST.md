...
- [~] Orion is mounted/wired; install final standalone `F-06` as `1x` Mersen `USM1` touch-safe `10x38 mm` DIN holder with `1x` Mersen `ATM20 20A/600VAC/DC` fuse. Mount the holder on a short secured `35 mm` DIN rail close to the Lynx source, land both existing `6 AWG` conductors directly in its pressure-plate terminals, torque to the current Mersen datasheet (`~15 lb-in` baseline), tug-test, and keep the unfused source-side lead short. Do not use the purchased housing-only FKS parts or add pigtails/reducer splices.
