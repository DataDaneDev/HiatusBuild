...
- Solar remains a discovery/provisioning workstream, while the Standard 4 X is in hand. The two existing longitudinal roof tracks already serve as the outer rails; use movable extrusion crossbars rather than duplicating a full perimeter. Current moving-route lead is two removable retractile jumpers—shielded Ethernet/PoE and separately rated PV—mounted side by side with independent structural strain relief. The coils store roof-down slack; a loose sleeved loop and an exposed side-mounted energy chain are no longer defaults because neither solves down-state projection/removal cleanly. Preserve the complete factory Starlink cable as an unmodified recovery spare. No roof penetration or final jumper/connector/OCP buy is released until down/up anchor geometry and the actual array `Voc`/`Isc`/stringing are measured. See [STARLINK_SOLAR_MOVING_UMBILICAL](STARLINK_SOLAR_MOVING_UMBILICAL.md).
...
5. **Prototype the removable twin-coil Starlink/PV moving section without a roof penetration:** measure actual fixed-to-moving-anchor travel, tangent/strain-relief lengths, allowable down-state coil body/projection, and connector-entry envelope. Bench-test the Type-4/RJ45 adapter plus shielded coil under live Starlink load before installing it; preserve the complete OEM cable as a recovery spare. Size the solar coil and two-pole DC PV disconnect only after panel/string cold `Voc` and `Isc` are locked. Prove roof-down, intermediate, roof-up, normal-road, and powered-down trail-removal states before final holes.
