...
- [~] First clean-water function pass completed: the `12V` pump, accumulator, tank, interior PEX, and open ball-valve endpoints operated, with no visible leaks observed. Repeat the test after the fill/rear-port fittings are installed because intentional ball-valve discharge wet the compartment and obscured a conclusive dry-joint inspection; verify gravity fill/vent, pump cycling, accumulator behavior, pump-assisted tank emptying, and every fitting/seam.
...
- [~] Hot/cold PEX reaches the sink zone and terminates at accessible ball valves/final `90` fittings. Install the sink/faucet, then terminate the faucet hoses with the purchased EFIELD `1/2 in PEX-B x 3/8 in OD compression male` adapters; use no PTFE tape at either compression seat.
...
- [~] Initial pressure test found no visible leaks, but open-valve discharge prevented a conclusive dry inspection. Before panel closure, dry the compartment completely, pressurize to pump cutoff with every outlet closed, place dry paper indicators at each joint, verify no unexplained pump cycling, and repeat inspection after dwell and pressure cycles.
...
- [ ] [HOLD] When the HOTTAP arrives, bring one actual hose and its supplied double-male coupler to the hardware store. Test Melnor `15409` first because Joolca documents Melnor QuickConnect compatibility; use Melnor plastic hardware as the profile control. Test GARDENA `39004-G`, Orbit/Gilmour/Morvat, and other metal families only as separate profiles. Also test the double-male coupler in a Melnor `4MQC` water-stop socket. Buy no two-port set and cut no camper hole until latch, seal, play, release, cap, hot-flow, drain, valve-access, panel-thickness, and backing tests pass.
...
- [ ] [HOLD] After the Melnor/Joolca fit passes, mock one approximately `30 in` potable-rated `3/4 in F-GHT x M-GHT` cold leader with Melnor `9MQC` at the female-thread end and `8MQC` water-stop at the male-thread end. Verify a relaxed service loop, BLUE labeling, flow/ignition, drainability, and cold-pressure integrity; do not cut the supplied red hose.
