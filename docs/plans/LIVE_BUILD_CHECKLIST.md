...
- Standard 4 X routing is now a one-panel package: Router 3 in an accessible ventilated Desk/service cubby, powered primarily by the selected dedicated-fuse `12V`-to-`57V` converter with factory AC/inverter fallback -> lower STARGEAR Type-4/RJ45 adapter at the proprietary router dish port -> fixed interior shielded pure-copper Cat6A -> one Neutrik `NE8FDX-P6-W` rugged shielded panel feedthrough -> `NE8MXR1-B-TOP-D`-terminated removable 3 m stretched shielded retractile roof jumper -> upper STARGEAR Type-4/RJ45 adapter -> terminal. A long outdoor shielded Cat6A cord can replace the roof jumper for ground deployment; ordinary RJ45 works at the panel in dry conditions, while wet use requires the compatible rugged/weather-sealed cable connector. Mount the dish in a TRIO Gen 3 Standard Speedmount across two black 10-series 1020 crossbars carried by physically verified Yakima `8810074` anchor plates, using four reusable fasteners/captured T-nuts; disconnect/remove the framed dish for tight brush because edge protection is not branch-proof. Preserve the complete factory cable as an unmodified recovery spare. Solar is explicitly deferred and will receive its own PV-rated route only after panel/string `Voc`/`Isc` are known. See [STARLINK_SOLAR_MOVING_UMBILICAL](STARLINK_SOLAR_MOVING_UMBILICAL.md).
...
7. **Keep Starlink/solar off the fabrication critical path:** the direct OEM-cable setup remains the first-trip fallback. Bench-fit the Type-4/RJ45 adapters, retractile jumper, Neutrik panel connector/cable carrier, and protective four-fastener crossbar mount before any wall or cable cut. Solar remains separate and array-gated; the current rotary service-disconnect lead is the IMO `SI32-PEL64R-2`, but final release still waits for cold `Voc`/`Isc` and stringing.
