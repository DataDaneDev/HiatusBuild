...
- Original reconciliation avoided double-counting by assigning the monitor arm to row `99` and the kept hardware remainder to aggregate row `222`. The `2026-07-26` BOM normalization preserved the `$774.23` hardware subtotal while splitting that aggregate across component rows `222` and `295-316`; July washer stock is now itemized under rows `243-244` rather than rolled into row `46`.
