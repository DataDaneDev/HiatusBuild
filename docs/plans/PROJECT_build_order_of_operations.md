...
- Starlink Standard 4 X, fridge, pump, three DC charging outlets, and the owner-described `12V`/`48V` systems are in regular use. All four duplex outlet boxes and remaining `120V` work are enclosed and in regular use; specific PE/chassis/shell bonding, GFCI/polarity, labeling, terminal-cover, and cable-support checks remain.
...
- Current dependency order is protective-earth bonding/GFCI proof -> heater/storage/tank package -> mid-September Bubinga/sink -> graywater -> PH-VAN/Wakespeed first charge -> solar bond/wire/commission -> HOTTAP on receipt -> Bench lid/air fryer/monitor/laptop/storage -> final skins and subsystem shakedowns.
...
- **Electrical safety task:** close the unresolved MultiPlus-to-truck-chassis and separate shell bonds, then prove PE continuity, neutral isolation, polarity, and both GFCI chains. Finish the labeled removable DC switch face, terminal covers, cable support, and labels where still open.
...
- Hold 4 (AC enclosures closed; bonding open): all four outlet boxes and remaining `120V` work are enclosed and in regular use. Close the MultiPlus-to-chassis and shell bonds plus PE continuity, neutral isolation, polarity, and GFCI proof before continued routine `120V` use.
