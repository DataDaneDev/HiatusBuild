...
- MultiPlus LiFePO4 charge profile is programmed/owner-verified by supervised first-battery behavior. Owner reports the `3x 48V` bank has been permanently paralleled and the `12V`, `48V`, and `120V` systems have been used regularly. Hold open only the explicit protective-earth/bonding and GFCI/polarity checks, alternator commissioning, any unfinished Cerbo/service-face mounting, and final strain-relief/abrasion-control.
...
- The driver-rear shore inlet and exterior-to-module cable route are installed. All `120V` work, including the four duplex outlet boxes, is owner-reported enclosed and in regular use. Preserve explicit PE/chassis/shell bonding, neutral-isolation, polarity, and GFCI checks.
