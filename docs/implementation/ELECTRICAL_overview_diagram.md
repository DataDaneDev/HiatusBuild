...
- Clarifies the confirmed `PH-VAN` harness as one `15A` high-interrupt in-line-fused combined power/positive-sense red lead (`F-12/F-13-PHVAN`) and locks the unfused purple/grey pair to the Wakespeed shunt on the battery side of the hard-mounted SmartShunt in the common battery-negative path.
...
## Current Commissioning Snapshot (`2026-09-02`)
...
- MultiPlus LiFePO4 charge profile is programmed/owner-verified by supervised first-battery behavior. Owner reports the `3x 48V` bank has been permanently paralleled, the `12V`, `48V`, and `120V` systems have been used regularly, and the MultiPlus is grounded to truck chassis. Hold open only inspection/continuity proof of that installed ground, the separate shell bond and GFCI/polarity/neutral checks, alternator commissioning, any unfinished Cerbo/service-face mounting, and final strain-relief/abrasion-control.

## Current Physical Installation Snapshot (`2026-09-02`)
- The electrical module is hard-mounted through the finished Lonseal/plywood floor into registered truck-bed hardpoints and tied into the hard-mounted Bench/Galley extrusion structure. Owner reports the batteries and ICECO are positively restrained; remaining mobile retention is the not-yet-installed air fryer, bench lid, monitor/laptop hardware, other storage/cargo, and any unfinished terminal/cable protection.
- The driver-rear shore inlet and exterior-to-module cable route are installed. All `120V` work, including the four duplex outlet boxes, is owner-reported enclosed and in regular use; the MultiPlus is owner-confirmed grounded to truck chassis. Preserve explicit verification of that installed `M6 PE` bond, the separate conductive-shell bond, neutral isolation, polarity, PE continuity, and GFCI checks.
- All three batteries' positive and negative `2/0 AWG` branch cables are complete and the bank has been permanently paralleled for an extended period.
...
- Owner report `2026-09-02`: Starlink Standard 4 X, ICECO/fridge, water pump, and the installed `12V` charging outlets have seen regular use. Starlink uses a dedicated `20A` blade fuse; the three separately fused charging outlets are driver-side `100W` and two passenger-side `65W`. Preserve exact slot/fuse labels, polarity, terminal covers, loaded voltage-drop/heat checks, and final switch/service-face mounting without reopening their functional status.
- Owner report `2026-09-02`: `C-31` Desk/office and `C-32` Galley/passenger branches each feed one origin GFCI plus one downstream duplex, for four duplex devices/eight plug positions total. Their boxes/enclosures and regular-use state are closed; retain explicit proof of breaker values, branch separation, `LINE/LOAD/PE`, downstream protection, polarity, and trip/reset in inverter and accepted-shore modes.
...
flowchart LR
    subgraph VEH_ALT48["Dedicated 48V Alternator Path"]
        ...
        WS500 -. "PH-VAN one 15A/80VDC fused power/sense lead;\nnegative-shunt current sense unfused" .- ALT48
    ...
    subgraph PV_PATH["Solar Path (received 700W / selected 4S1P)"]
        ...
        PVFIX["Two accessible sealed splices\nto stationary 12/2"]
        PVDISC["Two-pole PV load-break\nopen before service disconnect"]
        MPPT["Victron SmartSolar\nMPPT 150/45"]
        PVA -- "three series mates leave two end leads\n9.50A STC Isc / 114.86V cold screen\nhot-start proof pending" --> PVJ --> PVCORD --> PVFIX --> PVDISC --> MPPT
    end
...
- Continuous equipment grounding path from shore inlet through MultiPlus and branch circuits. Owner confirms the MultiPlus is grounded to truck chassis; verify at the next de-energized inspection that this is the external `M6 PE` connection with at least `4 mm²`, selected as `10 AWG` green stranded copper, to a sound chassis point. The aluminum shell gets a separate corrosion-compatible bond into the same equipment-ground network and is not the only PE path.
...
| `F-12/F-13-PHVAN` | Littelfuse `KLKD015.T`, `15A/600VAC/DC` fast-acting midget fuse | Eaton/Bussmann `HEB-AA` `600V` water-resistant in-line holder | Immediately beside the Lynx, fed from the `F-04` alternator/load-side stud through short `14 AWG` pigtails; sealed splice to `16 AWG` PH-VAN red; no panel, DIN rail, or enclosure |
...
| WS500 current-sense pair | No fuse; common battery-negative shunt with purple/high on battery-negative side and grey/low on SmartShunt/Lynx system side | Twist pair if extended; route away from noise; configure `Shunt at Battery` | Wakespeed shunt on the battery side of the hard-mounted SmartShunt |
...
| `C-27` | Purchased Renogy `4S` string -> two-polarity roof transition box -> continuous front-gap retractile -> fixed-shell premium gland -> two accessible sealed splices -> stationary `12/2` -> two-pole load-break -> MPPT PV input | `78.0V Vmp`; `95.6V Voc`; `114.86V` nominal-coefficient cold Voc at `-40C` | `8.98A Imp`; `9.50A Isc` STC; `14.8A` conservative conductor screen | No `F-09` presently planned for one string; reopen only from received-label/code review | Preferred `12 AWG x 2` fixed/moving PV path after final route/derating proof; moving cord requires written `>=150VDC`, coiled ampacity, outdoor/wet/UV/cold/flex proof. Current envelope is `~24 in` down / `~52 in` up; research `60-66 in` rated overall. No four-to-one branch/combiner. | No exterior PV quick disconnect; lengths/OD/tangents/clamps/guard/gland/box/splices remain on full-stroke and articulation HOLD; hot restart/tracking proof required |
...
| `C-38` | `F-04` alternator/load-side stud -> Eaton/Bussmann `HEB-AA` + Littelfuse `KLKD015.T` -> WS500 `PH-VAN` red lead | `48V` bank | Combined regulator-power / positive-voltage-sense feed | `F-12/F-13-PHVAN` (`15A`) | Short `14 AWG` pigtails, sealed transition to `16 AWG` harness | In-line beside Lynx/F-04; no panel or enclosure |
| `C-39` | Retired separate WS500 positive-sense conductor | N/A | Not installed with confirmed `PH-VAN` harness | None | N/A | N/A |
| `C-40` | WS500 current-sense pair to common battery-negative shunt: purple/high battery-negative side, grey/low SmartShunt/Lynx system side | low-current sense | Net battery-current feedback | No fuse; twist pair if extended; configure `Shunt at Battery` | Harness lead | Local near house board |
...
| `C-27` | Purchased Renogy `4S` string + two-polarity transition box + continuous front-gap retractile + premium gland + stationary `12/2` + two-pole disconnect | MPPT PV input | No `F-09` presently planned | `9.50A Isc` STC / `14.8A` conservative conductor screen | Preferred `12 AWG x 2` after route/derating proof | Full route pending; `20 ft` one-way screen | `0.94%` at `9.5A / 78V` for `12 AWG` | Rows `31`, `106`, `121` | HOLD received labels/connectors, direct measurement of the current `~24 in down / ~52 in up` endpoint estimate, tangent/transition-box/gland/splice geometry, `60-66 in` rated-overall working reserve, `>=150VDC` and coiled-ampacity proof, articulation/full-stroke mockup, and hot restart/tracking commissioning |
...
| `C-38` | `F-04` alternator/load-side stud | WS500 `PH-VAN` short red lead | `F-12/F-13-PHVAN` Littelfuse `KLKD015.T 15A/600VAC/DC` | combined regulator-power / voltage-sense feed at `48V` bank voltage | `14 AWG` pigtails to `16 AWG` harness | Local | N/A (harness-limited) | Active row `171`; Eaton/Bussmann `HEB-AA` in-line holder | VERIFY crimp, sealed splice, strain relief, and label |
...
| `C-40` | Common battery-negative Wakespeed shunt: purple/high battery-negative side, grey/low SmartShunt/Lynx system side | WS500 current-sense input | No fuse | low-current sense; twist pair if extended | Harness lead | Local | N/A (harness-limited) | Harness kit | VERIFY `Shunt at Battery`, SmartShunt remains hard-attached to Lynx, no shunt bypass, and positive current sign |
...
| `12 AWG` PV cable allowance | Route length pending | `C-27` purchased Renogy `4S`; three series mates, two-polarity roof transition, continuous front-gap wet/flex retractile, stationary straight lower gland tail, then two sealed splices to stationary `12/2`; no four-to-one combiner or `F-09` presently planned | `31`, `106`, `121` |
