...
8. Two CPC `HFC16835 NSF` / ordering number `86600` straight-through panel-mount bodies with `1/2 in` hose barbs.
9. Two CPC `HFC22835 NSF` / ordering number `83100` straight-through inserts with `1/2 in` hose barbs, one per removable umbilical.
10. Two CPC `HFC312L` tethered dust plugs for the panel bodies; these are contamination covers, not pressure shutoffs.
11. One custom cold umbilical: CPC insert, `1/2 in ID` reinforced hose, clamp at the CPC barb, and the Joolca-compatible blue/Melnor inlet connector at the heater end.
12. One custom hot-return umbilical: Joolca red outlet connector or spare hose assembly, `1/2 in ID` reinforced hose, CPC insert, and the adapters/clamps proven by the in-hand Joolca fitting. The supplied shower hose remains unmodified for shower mode.
13. At least four additional correctly sized ear clamps for the two umbilicals if both heater-end connectors terminate in hose barbs; lock the exact count after the Joolca connectors are in hand.
14. One thin aluminum two-port service plate, backing, butyl bedding, perimeter sealant, a sealed wall-core sleeve/grommet, BLUE/RED labels, weather cover, and two clean loose-hose caps for storage.
15. Twelve `1/2 in` PEX cinch clamps downstream of the accumulator: one at the accumulator adapter, three at the tee, one at each faucet adapter, two at each service valve, and one at each `UP120A5` transition.
16. Potable-water-rated PTFE tape or thread sealant on the two `UP120A5` MNPT-to-Anderson FNPT joints only; do not apply thread sealant to compression seats, hose barbs, QD O-rings, or Joolca gasketed/BSP hose swivels unless that fitting's instructions require it.
...
The heater stores inside the bumper box but operates outside its storage envelope. Use Joolca's removable bracket, supplied feet, or a positively locked outboard arm so the unit is vertical in free air. Joolca's published operating-clearance baseline is at least `39 in` above, `23.6 in` behind, and `19.7 in` at the front and both sides. The quick-release bracket is approved for mounting the unit to a vehicle or flat surface, but Joolca separately says the clearance instructions on the unit still apply during use; do not assume the box skin or open lid is a bracket-specific rear-clearance exception without written Joolca confirmation. Therefore the default proof path is feet/stand or an outboard arm that physically passes the clearance envelope. No heater, water, or connected LP hose crosses the moving swingout pivot during travel.

Use one upright `5 lb` DOT cylinder with QCC1/Type-1 valve; the selected basis is Flame King `YSN5LB`. Do not add a separate regulator or gauge: the HOTTAP includes the correct regulator/hose, and an inline pressure gauge adds a leak path without accurately measuring remaining propane. NFPA 1192 says an RV propane container shall not be mounted on the exterior rear wall or bumper. The conservative travel baseline is therefore a disconnected, valve-closed, upright restraint **forward of the camper's rear plane** on a frame/bed-structure-tied open-air bracket, or another appropriate open cargo location accepted by the applicable RV/vehicle authority. The rear swingout box is not the default filled-cylinder travel location. At camp, remove the cylinder to a stable ground cradle, keep the LP hose fully outdoors, and do not place the cylinder below the heater or in its exhaust/clearance envelope. If no compliant travel location can be proven, the cylinder-transport architecture remains blocked even though the heater and water architecture are selected.
