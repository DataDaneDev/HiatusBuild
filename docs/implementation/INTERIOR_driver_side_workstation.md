...
- Seal the turret as two boundaries. Keep the factory heater gasket between heater and turret. Under the turret flange at the finished Lonseal plane, use a thin continuous formed-in-place gasket of Permatex Ultra Copper high-temperature RTV, inside the fastener line and looped around each fastener hole; mechanically fasten evenly for light squeeze-out and allow the full `24 h` cure before heating. Lonseal cautions that silicone may not always bond to vinyl and may discolor it, so keep this bead hidden and mechanically compressed rather than treating it as exposed flooring caulk. Seal the stainless skirt to the cut aluminum bed separately with a continuous `360-degree` Ultra Copper joint, including rib peaks and valleys; if any annular gap exceeds the product's roughly `6 mm / 1/4 in` published gap fill, stop and add a thin noncombustible metal collar/backer rather than bridging it with RTV, foam, wood, or EPS. The diesel-rated Permatex thread sealant purchased for tapered NPT fuel joints is not the turret sealant.
- Outside only: connect the supplied corrugated exhaust to one stub of the supplied two-stub oval LF Bros muffler and use the other bare stub as the terminal outlet; the official LF Bros connection video does not cut the long pipe in half or add a downstream tail. It makes only a short longitudinal relief cut at the pipe end for fit-up. Use the metal clamp and hard-mount the muffler by its bracket rather than leaving it dangling; follow any actual flow arrow and point any condensate drain down. Keep a continuous slight fall for condensate and use broad sweeps rather than a hard `90-degree` elbow at the heater. Keep the muffler and entire exhaust path outside, separated and shielded from fuel line, pump, undercoating, wiring, plastic, openings, and the combustion intake. Aim the final outlet away from the intake and occupied openings.
