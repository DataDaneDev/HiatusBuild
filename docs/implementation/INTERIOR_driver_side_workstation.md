...
- Purchased heater baseline is the LF Bros `5 kW / 12V / 10 L Split Pro` kit with T4S hardwired LCD controller and wireless remote. The heater body is approximately `14.6 L x 3.5 W x 5.9 H in`; preserve the real connectors, clamps, service loops, and insulation/guarding beyond that bare envelope.
...
- The supplied `10 L` tank does **not** accept the pump in the molded circular pad. Mount the tank upright with fill/vent cap high; drill only a centered `7-8 mm` hole through the molded lower outlet boss for the supplied bulkhead/nozzle. Pull the nozzle through from inside using the small fuel line as a leader, then install the outer seal/washer/nut. Do not remove the full `3/4-1 in` molded pad.
- Fuel order is `tank bottom outlet -> filter -> external metering pump -> heater`. Mount the supplied pump in its rubber clamp close to the tank, within `2 m` of the heater, with the **outlet toward the heater and angled upward about 45 degrees**. Keep the tank, fill, filter, and pump outside the open battery/electrical/living bay or in sealed externally vented containment; only a protected small fuel line should enter the heater zone.
