...
| `F-09` (not presently required) | Selected Arch Pro `3S2P` string positives -> branch pair or two-string combiner -> MPPT PV input | Add only if the received-module/code/backfeed review requires individual string OCP | Listed `gPV`, voltage class above the final `142.29V` cold string calculation | No fuse in the current two-string design; if required later, size from received `Isc`, `15A` module series-fuse rating, conductor ampacity, and applicable PV rules | Compatible branch connectors or a listed two-string combiner; do not procure the retired three-position fuse package | Roof-entry/service area | Final gauge after moving-route measurement; `10 AWG` PV cable is the planning class |
...
| PV string OCP (`F-09`) | `0` planned | `0` | Selected array has only two parallel `3S` strings. One peer string cannot backfeed enough current to approach the Arch Pro `15A` series-fuse rating. Reopen only if received labels, conductor/connector limits, or applicable PV rules require it. |
...
2. Solar topology is locked to `6x BougeRV Arch Pro 100W`, `3S2P`, on the Victron `150/45`. Each string is `113.4V Voc` and `3.2A Isc` at STC; the tolerance-aware cold calculation is `142.29V` at `-40F`. With only two parallel strings, no individual `F-09` is presently planned. Verify the received module labels, connector/conductor ratings, moving-route ampacity, and final code basis before as-built release; do not purchase or install the retired `3x 15A gPV` package.
