...
| `F-06` | Lynx `48V +` bus tap -> Orion `48V` input `+` | Orion `48V` input/device lead | **Interim/current:** existing `30A 58V` MIDI on the short `6 AWG` Orion input run. **Final planned purchase:** Mouser `576-166.7000.5202` Littelfuse FKS/ATO blade fuse (`20A`, `80VDC`) in Mouser `576-178.6150.0001` Littelfuse ATO/FKS holder (`80VDC` per owner-confirmed listing). | Interim `30A`; final `20A` per Victron Orion `48V` external battery-protection recommendation | Interim MIDI holder now acceptable for build/bench use; final FKS/ATO holder later from Mouser. | Electrical cabinet between Lynx `+` bus and Orion input | `6 AWG` planned; pigtail/transition acceptable if the final blade holder does not directly accept `6 AWG` |
...
| Orion `48V` input protection | Interim: existing `30A 58V` MIDI. Final: Mouser `576-166.7000.5202` `20A 80VDC` FKS/ATO fuse + `576-178.6150.0001` `80VDC` holder | Carry `2` spare final `20A` FKS/ATO fuses after purchase | Interim `30A` protects the short `6 AWG` run; final `20A` matches the Orion manual and is the preferred cleanup purchase |
