...
| `F-05` | Lynx Slot 4 -> Orion `48V` input branch | Positive feeder from Lynx Slot 4 toward the Orion `48V` input and, if correctly sized, the Orion input/device protection | `MEGA`, `58V` or `80V` | Preferred: Victron Orion `48V` external battery-protection value if available in a Lynx-compatible fuse; do not treat the existing `40A` stock as final Orion protection | Integrated Lynx Distributor fuse slot | Lynx Distributor, Slot 4 | `6 AWG` |
