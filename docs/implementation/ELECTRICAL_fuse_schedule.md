...
| `F-05` | Lynx Slot 4 -> Orion `48V` input branch | Optional source-side Orion input fuse location if a practical `20A` Lynx-compatible MEGA/MIDI solution is confirmed | `MEGA`, `58V` or `80V` | Target `20A` per Victron; existing `40A` stock protects the `6 AWG` branch but is not the preferred Orion device-fuse value | Use only if the slot can be populated with a practical `20A` fuse; otherwise use standalone `F-06` | Lynx Distributor, Slot 4 | `6 AWG` planned if used |
| `F-06` | Lynx `48V +` bus tap -> Orion `48V` input `+` | Orion `48V` input/device lead; low-cost standalone source-side fuse if Lynx Slot 4 cannot provide a `20A` device fuse | Preferred practical path: Littelfuse `166.7000.5202` FKS/ATO blade fuse (`20A`, `80VDC`) in an ATO/FKS holder with a visible `80VDC` rating. Candidate holder family: Littelfuse FH2/`178.6152.0001`; verify terminal style/gauge. Avoid holders that are only `32VDC` rated. | `20A` per Victron Orion `48V` external battery-protection recommendation | Compact inline or block holder mounted near the source end of the Orion input branch. Existing `30A 58V` MIDI stock is acceptable bench/contingency wire protection for `6 AWG`, but the cheap final target is `20A`. | Electrical cabinet between Lynx `+` bus and Orion input | `6 AWG` planned; pigtail/transition acceptable if the selected blade holder does not directly accept `6 AWG` |
...
| Orion `48V` input protection | `1` total source-side fuse: preferably `20A` Littelfuse FKS/ATO `80VDC` hardware (`F-06`) unless a practical `20A` Lynx-compatible option is sourced | `2` of the final value | Existing `30A 58V` MIDI stock can protect `6 AWG` as bench/contingency stock, but `20A` better matches the Orion manual |
