...
| `F-09` (quantity TBD) | Final PV string positive leads -> required PV OCP/combiner or junction -> MPPT PV input | PV string conductors and module reverse-current exposure if final parallel-string design requires individual OCP | Listed `gPV`, voltage class at or above final maximum cold string `Voc`; exact fuse type/rating unlocked | TBD from exact module `Isc`, maximum-series-fuse rating, parallel-string count, and applicable PV rules | Listed PV-rated holder/combiner only if required; do not procure the old three-position package | Roof-entry/service area, final location unlocked | Final PV conductor gauge unlocked |
...
| PV string OCP (`F-09`) | Quantity TBD | Carry spares only after final fuse family/rating is locked | The current candidates are one `4S` Solbian string or two dissimilar-current `4S` Lensun strings; individual string fuses may not be required. Decide from exact module and applicable PV data, not the retired `3S3P` count. |
...
2. `F-09` quantity and rating are unlocked. The `2026-08-09` roof audit left a one-string `4x Solbian SP138 / 4S1P` fallback and a two-string `4x130W + 4x70W / 4S2P` high-output candidate. Determine whether individual string fuses are required from the final module `Isc`, maximum-series-fuse rating, parallel-string reverse current, conductor ampacity, and applicable PV rules. Do not purchase or install the retired `3x 15A gPV` package.
