...
| `F-05` | Lynx Slot 4 -> Orion `48V` input branch | Positive feeder from Lynx Slot 4 toward the Orion `48V` input and, if correctly sized, the Orion input/device protection | `MEGA`, `58V` or `80V` | Preferred: Victron Orion `48V` external battery-protection value if available in a Lynx-compatible fuse; otherwise lowest suitable Lynx branch value pending `F-06` decision | Integrated Lynx Distributor fuse slot | Lynx Distributor, Slot 4 | `6 AWG` |
| `F-06` | Lynx Slot 4 Orion feeder -> Orion `48V` input `+` | Orion `48V` input/device lead only if `F-05` cannot be sized to satisfy the Orion input-protection requirement | Inline fuse/holder rated above the actual `48V` bank maximum (`80VDC` preferred; `58VDC` class only if manufacturer confirms suitability for the `58.4V` charge profile) | Same value as Victron Orion `48V` external battery-protection recommendation unless manual/vendor approval changes it | Conditional sealed inline holder near the source/Orion backplate; omit if `F-05` is correctly sized and physically adjacent | Electrical cabinet, only if the Lynx fuse cannot serve as the sole Orion input fuse | `6 AWG` planned (`8 AWG` minimum per Orion terminal table; Victron table allows smaller for short 48V runs) |
...
| Orion `48V` input protection | `1` total source-side fuse, preferably `F-05` in Lynx Slot 4 if the correct value exists for Lynx | `2` of the final value | Do not stack a redundant inline `F-06` beside the Lynx unless Lynx Slot 4 cannot be populated with the Orion-required value/rating; purchased `30A` MIDI stock is not install-locked |
...
7. Orion `F-05/F-06` reassessment: the desired final topology is one source-side Orion input fuse, not two adjacent fuses. Use Lynx Slot 4 alone if a correctly rated/value fuse can satisfy the Orion input-protection requirement; keep separate `F-06` only if Lynx cannot be populated with that value/rating.
