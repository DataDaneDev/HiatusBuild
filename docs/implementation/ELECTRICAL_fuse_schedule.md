...
| `F-06` | Lynx `48V +` bus tap -> inline fuse -> Orion `48V` input `+` | Orion `48V` input/device lead; this is the one source-side Orion input fuse | **Interim/current:** existing `30A 58V` MIDI on the short `6 AWG` Orion input run. **Final fuse stock purchased:** `3x` Mouser `576-166.7000.5202` / Littelfuse `166.7000.5202` FKS/ATO blade fuses (`20A`, `80VDC`). **Holder correction:** purchased `576-178.6150.0001` / `178.6150.0001` parts are empty housings, not complete holders. | Interim `30A`; final `20A` per Victron Orion `48V` external battery-protection recommendation | Final path: obtain `1x` complete `178.6152.2501` holder with `200 mm`, `2.5 mm²` loop, cut loop at midpoint, and use `2x` purpose-built adhesive-lined `6 AWG -> 14-16 AWG` reducer butt splices (Ancor `309106` class). Keep the unfused source-side tap from Lynx to `F-06` physically short and protected. | Electrical cabinet between Lynx `+` bus tap and Orion input | Keep existing `6 AWG`; transition only through the complete holder's short `2.5 mm²` pigtails and rated reducer splices |
...
| Orion `48V` input protection | Interim installed/available: existing `30A 58V` MIDI. Correct final fuse stock: `3x` Mouser `576-166.7000.5202` `20A 80VDC` FKS/ATO fuses. Purchased `3x` `576-178.6150.0001` parts are empty housings, not complete holders. | Install `1x` correct fuse in `1x` complete `178.6152.2501` holder using `2x` rated `6 AWG -> 14-16 AWG` reducer splices; carry `2` spare final `20A` fuses | Interim `30A` protects the short `6 AWG` run; final `20A` matches the Orion manual. Do not count the housing-only parts as spare holder assemblies. |
...
9. Orion holder correction: `178.6150.0001` is the housing only. Its compatible standalone DFK contact family accepts `1.5-2.5 mm²`, so it cannot receive the existing `6 AWG` directly. Use a complete holder assembly and rated gauge-transition hardware; do not improvise by trimming strands or forcing `6 AWG` into the housing/contact system.
