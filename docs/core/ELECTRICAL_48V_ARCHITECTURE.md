...
- Owner reports regular use of the `12V`, `48V`, and `120V` systems. All four duplex outlet boxes and the other `120V` enclosures are complete, and the owner confirms the MultiPlus is grounded to the truck chassis. Remaining AC safety verification is explicit inspection/continuity proof of the installed MultiPlus `M6 PE` bond, the separate conductive-shell bond, GFCI `LINE/LOAD`/downstream trip behavior, polarity, PE continuity, and neutral-isolation behavior—not another enclosure or chassis-ground build.
...
- **AC protective earth:** the MultiPlus external `M6 PE` lug is mandatory in this mobile installation. Owner report `2026-09-02`: the MultiPlus is grounded to truck chassis, closing physical installation of that bond. At the next de-energized inspection, verify the connection originates at the `M6 PE` lug, uses at least the Victron-manual `4 mm²` conductor (current build default `10 AWG` green stranded copper), and lands at a sound truck-chassis bond point. Do not use the aluminum camper shell or 80/20 as the only protective-earth conductor.
