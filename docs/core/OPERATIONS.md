...
- Safe energization order (DC): keep `48V` disconnect open and remove all Lynx branch fuses including Orion `F-05`; connect negatives first (battery negatives to battery-side negative combine, then to SmartShunt battery side); wire positives to Class T blocks with Class T fuses removed.
...
- System bring-up: close `48V` disconnect to energize the Lynx bus; then insert active Lynx branch fuses one-at-a-time (`F-02`, `F-03`, `F-04`, and `F-05` as applicable), verifying each branch after insertion. `F-03` in Slot 2 must be `60A/80V`; `F-05` in Slot 4 must be `40A`, body-marked at least `58VDC`, and directly energizes Orion. Standalone `F-06` is retired. No `32V`-rated fuse is acceptable on the energized `48V` bus.
...
- Supervised verification: settings were redone with the current target and the first battery behaved as planned, entering bulk and then quickly transitioning to absorption because it was already at/near `100%`. Earlier settled snapshot with shore input limited to `12A` showed MultiPlus and SmartShunt at `56.8V` in absorption with `0A` charge current; battery display read about `55.8V`; owner set SmartShunt SOC to `100%` at this settled state. On `2026-08-02`, MK3 inspection reconfirmed the voltage profile, corrected the isolated-battery charger-current cap from `35A` to `20A`, and Battery 1 again completed a clean `56.76V` absorption / `54.06V` float cycle without an observed BMS trip. Battery 2 then charged normally at about `53.75V` / `19A`. SmartShunt SOC is not valid across physical battery swaps until re-synchronized at a known full state.
- Historical alarm interpretation (`2026-08-02` review): SmartShunt retained a `58.96V` maximum and `6` high-voltage alarms from the earlier May testing. Owner-observed timestamps were roughly `10 minutes` apart with `5-10 minute` clear/recovery intervals, consistent with a repeated internal-BMS overvoltage disconnect/recovery/recharge loop. Treat that as a strong inference rather than definitive BMS-log proof; do not use the BMS protection ceiling as routine charge control.
