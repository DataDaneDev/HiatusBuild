...
- Hot-water decision posture (`2026-07-25`, final): propane only. Selected appliance basis is the outdoor-use Joolca HOTTAP V2 Essentials (`37,500 BTU/hr`, `0.6-1.6 GPM`, recommended source `40-60 PSI` and `1.6-3.17 GPM`, about `17.7 x 12.2 x 6.7 in`) with its supplied four-foot QCC1 regulator/hose and one `5 lb` QCC1 cylinder. It stores in the rear swingout box and deploys vertically outside the box for operation. The fixed camper interface is `accumulator -> 1/2 in PEX tee`; one branch feeds faucet cold and the straight leg runs through a BLUE service valve/QD to the heater. Heater hot returns through a RED QD/service valve and insulated `1/2 in` PEX to faucet hot; shower mode substitutes the supplied shower hose at the heater outlet, so there is no exterior selector valve. Source/fitting details and the complete inline count live in [INTERIOR_furniture_layout_and_galley](../implementation/INTERIOR_furniture_layout_and_galley.md#propane-hot-water-routing-baseline-2026-07-25-final-direction).
- Rear propane package: HOTTAP stores dry inside the existing box but is lifted/hooked onto an exterior bracket or free-standing feet for operation; the open storage volume is not the combustion envelope. The `5 lb` cylinder is portable cargo, valve-closed and upright for travel, then removed to stable ground for use. Joolca's supplied QCC1 regulator/hose is the only LP hose/regulator; do not add a gauge, permanent rear-bumper LP line, or connected hose across the swingout pivot. NFPA 1192's rear-wall/bumper container prohibition is the reason the cylinder is not hard-mounted as vehicle LP supply.
