...
- Solar lead path remains sizing-gated on final panel/string cold `Voc`, `Isc`, route length, and roof travel. igus `CF9-UL-60-04` is straight four-conductor chain cable rather than a retractile jumper and is removed from the lead moving-section BOM. Use a purpose-built two-conductor outdoor/wet/repeated-flex coil only after calculations. Open a correctly rated two-pole DC PV load-break before disconnecting matched-brand MC4 or other service connectors.
