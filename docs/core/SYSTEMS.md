...
- Solar lead path uses a continuous retractile cord in the camper-to-cab gap, with no exterior quick disconnect and no manual roof-motion step. The helix stays entirely outside; only a stationary factory straight lower tail passes through a premium fixed-camper compression gland, with structural jacket clamps outside and inside so the gland/terminals carry no spring load. After the inside clamp, exactly two accessible sealed splices transition to stationary `12/2`; no long retractile tail is needed inside. Preliminary clamp-center span is about `3 ft` down to `7 ft` up. TPC `60241` proves that its `1-5 ft` active helix plus two additional `12 in` tangents is `3-7 ft` overall, but that exact assembly would be at hard extension at the estimated roof-up span and is therefore only the geometry reference. Preferred buy direction is custom black `12/2`, about `3 ft` overall retracted and roughly `8.5-9 ft` rated overall after field measurement, with `>=150VDC`, outdoor/wet/UV/cold/coiled-ampacity and force proof. No four-to-one branch/combiner is used: three series mates leave only two string-end leads, which transition to the retractile in a compact two-polarity box. LAPP `53112025` M20 long-thread brass is the premium gland lead for a measured `7-13 mm` cord; a roughly `16 mm` SOOW fallback would require the M25 `53112035`. Open the IMO `SI32-PEL64R-2` two-pole DC-PV load-break before any de-energized matched-brand solar service connection.
