...
- Hot-water decision posture (`2026-07-26`, owner-corrected final): propane only. Selected appliance is the outdoor-use Joolca HOTTAP V2 Essentials specified by the owner at `35,000 BTU/hr` and `2.5-6 L/min` (`0.6-1.6 GPM`), with a `1.2 m / 3.9 ft` LPG hose, GASKNECT gas coupling, and `1/2 in BSP` water quick-connects. Mount it directly to a structurally backed exterior side face of the rear box with Joolca's vehicle quick plate and mounted cover; it remains outside the cylinder compartment and no articulating arm or box-side water plate is used. The camper-port direction is two compact Koolance `QD3-FT10-P` female no-spill panel couplings, labeled BLUE cold out and RED hot return. Two removable adapter keys transition the QD3 males to the Joolca-compatible garden QuickConnect male profile accepted by the supplied female/female hoses; no threaded adapter is installed or removed during routine camp setup. The supplied `1 m` hose reaches the measured approximately `19 in` camper-to-heater span with margin; use the `4 m` hose from heater outlet either to the shower handle/head or camper RED hot return. Joolca explicitly documents Melnor QuickConnect compatibility and connection to an existing kitchen faucet. Source/fitting details and the complete inline count live in [INTERIOR_furniture_layout_and_galley](../implementation/INTERIOR_furniture_layout_and_galley.md#propane-hot-water-routing-baseline-2026-07-26-owner-corrected-final-direction).
...
- HOTTAP mounting/operation: Joolca explicitly permits vehicle mounting. Through-bolt its vehicle quick plate to proven structural backing on the exterior box side; do not ask the aluminum skin alone to carry road vibration. Keep the mounted cover on for travel/dust protection and remove/open it as Joolca requires before operation. Disconnect LP and water for travel. Before first ignition, prove the actual exterior location against Joolca's published operating clearances and confirm that exhaust/heat do not impinge on the camper, box door, cylinder compartment vents, stored gear, or cover. Never burn the HOTTAP inside the cylinder box, even with its large door open.
- Freeze and winterization strategy (`2026-07-26` final): after every freezing-weather use, close LP, isolate/relieve the active water path, disconnect and gravity-drain both Joolca hoses and removable adapter keys, then follow the HOTTAP manual's drain/storage procedure. Keep both QD3 service ports behind accessible camper ball valves and arrange the short reinforced-hose transitions to drain without a trapped low point. Clean the recessed female sockets and fit simple measured stretch caps for travel. Use tank drain/open fixtures and regulated blowout only if needed; no permanent antifreeze pickup or three-valve heater bypass.
...
- Current plumbing-route concept (`2026-07-26` final propane baseline): draw from a low north-end port through the existing tank shutoff, full-flow 90 only if the real turn requires it, flex, strainer, pump, discharge flex, and accumulator. Mount the purchased SHURflo/SEAFLO pack in the measured `6 in` cooler-to-battery gap on a thin vibration-isolated plate supported by extrusion; preserve strainer-bowl removal and point fittings/leak paths away from the batteries behind a nonconductive splash divider. The SEAFLO `SFAT-075-125-01` accumulator has two `1/2 in MNPT` pass-through ports; its supplied white swivels are flexible-hose barbs, not PEX-B. Use the purchased RecPro double-FIP hose on the pump side and one accepted YVSKM female-swivel PEX-B adapter on the outlet. After the accumulator, one `1/2 in` PEX tee branches to faucet cold while the straight leg continues through an accessible BLUE full-port valve, existing `UP120A5` `1/2 in PEX x 1/2 in MNPT` adapter, `1/2 in FNPT x 3/8 in` hose barb, and short supported hot-water-capable reinforced hose to one Koolance `QD3-FT10-P` female no-spill panel coupling. A second identical QD3 assembly through an accessible RED valve and insulated `1/2 in` PEX feeds faucet hot. The QD3 bodies and matching `QD3-MTN14` male adapter keys automatically obstruct both sides when disconnected, eliminating routine ball-valve operation. Each removable key permanently assembles `QD3-MTN14 -> 1/4 in FNPT x 3/4 in MGHT brass reducer -> proven Joolca-compatible male garden QuickConnect`; test Melnor brass `15409` first and use the plastic `2MQC` only as a removable-key fallback if the brass profile does not fit. Use the supplied `1 m` Joolca hose from BLUE to the heater inlet if it reaches without strain; use the supplied `4 m` hose from heater outlet to either the shower handle/head or RED hot return. No box-side plate, CPC jumper set, arm service loops, permanent three-way splitter, electric heater branch, manifold, bypass, or concealed joint behind the tank remains. Keep the gravity tank drain and sink gray drain physically separate from both pressure-service ports; treat the external hose/hot branch as recreational wash water rather than drinking-water hardware.
...
- Mount Joolca's vehicle quick plate directly to proven structural backing on the exterior box side, use the mounted cover for travel/dust protection, and prove the manual clearances in the installed operating position.
