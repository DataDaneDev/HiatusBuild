...
- Interior layout and wet-spine draft owner: [INTERIOR_furniture_layout_and_galley](../implementation/INTERIOR_furniture_layout_and_galley.md). Current direction is a passenger-side lofted fridge/wet-spine exoskeleton: Iceco/fridge raised about `16 in`, pump/accumulator relocated into the owner-measured `6 in` gap between the cooler support and house batteries, rear fill/vent and BLUE/RED hot-water service access, graywater cassette, and no interior water-heater volume. The wet pack requires a nonconductive splash divider and drained leak path toward the bed floor, not reliance on the battery cases as water protection.
...
- Hot-water decision posture (`2026-07-25`, final): propane only. Selected appliance basis is the outdoor-use Joolca HOTTAP V2 Essentials (`37,500 BTU/hr`, `0.6-1.5 GPM`, recommended source `40-60 PSI` and `1.6-3.17 GPM`, about `17.7 x 11.4 x 6.6 in`) with its supplied four-foot QCC1 regulator/hose and one `5 lb` QCC1 cylinder. It stores in the rear swingout box and deploys vertically outside the box for operation. The fixed camper interface is `accumulator -> 1/2 in PEX tee`; one branch feeds faucet cold and the straight leg runs through a BLUE service valve/QD to the heater. Heater hot returns through a RED QD/service valve and insulated `1/2 in` PEX to faucet hot; shower mode substitutes the supplied shower hose at the heater outlet, so there is no exterior selector valve. Source/fitting details and the complete inline count live in [INTERIOR_furniture_layout_and_galley](../implementation/INTERIOR_furniture_layout_and_galley.md#propane-hot-water-routing-baseline-2026-07-25-final-direction).
- Rear propane package: HOTTAP stores dry inside the existing box but is lifted onto feet, a stand, or a positively locked outboard arm for operation; the storage volume is not the combustion envelope. Prove Joolca's published `39 in` top, `23.6 in` rear, and `19.7 in` front/side operating clearances rather than assuming an open lid or bracket against the box wall is an exception. The `5 lb` cylinder is disconnected, valve-closed and upright for travel in an accepted open-air restraint forward of the camper's rear plane or other approved open cargo location, then removed to a stable ground cradle for use. The rear swingout box is not the default filled-cylinder travel location. Joolca's supplied QCC1 regulator/hose is the only LP hose/regulator; keep it outdoors and do not add a pressure gauge, permanent rear-bumper LP line, or connected hose across the swingout pivot.
- Freeze and winterization strategy (`2026-07-25` final): after every freezing-weather use, close LP, close BLUE, relieve pressure at the active outlet, close RED, disconnect and gravity-drain both umbilicals, and follow the HOTTAP manual's drain/storage procedure. Keep the two CPC panel bodies straight-through/non-valved and the interior service valves accessible so exterior stubs drain. Use tank drain/open fixtures and regulated blowout only if needed; no permanent antifreeze pickup or three-valve heater bypass.
...
- Current plumbing-route concept (`2026-07-25` final propane baseline): draw from a low north-end port through the existing tank shutoff, full-flow 90 only if the real turn requires it, flex, strainer, pump, discharge flex, and accumulator. Mount the purchased SHURflo/SEAFLO pack in the measured `6 in` cooler-to-battery gap on a thin vibration-isolated plate supported by extrusion; preserve strainer-bowl removal and point fittings/leak paths away from the batteries behind a nonconductive splash divider. After the accumulator, one `1/2 in` PEX tee branches to faucet cold while the straight leg continues to the rear BLUE service valve/QD. The separate RED rear QD/service valve feeds one insulated, joint-minimized `1/2 in` PEX line to faucet hot. Both QDs mount in a thin backed exterior service plate rather than through-clamping the sandwich wall. There is no manifold, bypass, permanent interior heater branch, exterior selector, or concealed joint behind the tank. Keep the gravity tank drain and sink gray drain physically separate from both pressure-service ports.
