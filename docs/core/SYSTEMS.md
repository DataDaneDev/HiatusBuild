...
- Near-term baseline: complete the cold-water galley and keep the selected heater as a simple parallel branch. Build around tank, pump, faucet/sink, drain/graywater path, service shutoffs, and the measured side-port heater envelope.
- Interior layout and wet-spine draft owner: [INTERIOR_furniture_layout_and_galley](../implementation/INTERIOR_furniture_layout_and_galley.md). Current direction is a passenger-side lofted fridge/wet-spine exoskeleton: Iceco/fridge raised about `16 in`, pump/accumulator relocated into the owner-measured `6 in` gap between the cooler support and house batteries, rear shower/fill/vent service access, graywater cassette, and the side-port storage heater in the adjacent cubby. The wet pack requires a nonconductive splash divider and drained leak path toward the bed floor, not reliance on the battery cases as water protection.
...
- Hot-water decision posture (`2026-07-25`): the current hard gate is `17 in` total vertical clearance, and all tanked candidates must remain upright. The new lead is Ariston ARI POU-06 / Amazon `B0924JBLT2` (`6 gal`, `120V`, `1500W`, `12.5A`): the [official manual](../../references/Ariston_ARI_POU-06_Use_and_Care_Manual.pdf) confirms a `13.8 in` jacket diameter, `15.8 in` shell height, `16.175 in` floor-to-top-water-connection height, `3/4 in NPT` top and side hot/cold connections, side T&P location, and side drain. Using only the side water ports leaves `0.825 in` theoretical vertical margin under the measured `17 in` cap; top-port caps remain, and the top anode/electrical area requires extracting the heater for uncommon service. If the earlier `27 x 15 in` horizontal envelope still applies, orient the hot/cold versus T&P/drain port axis along the `27 in` direction: the `13.8 in` shell then leaves `13.2 in` total for opposing side fittings, while the narrow axis has only `1.2 in` total shell clearance. Owner cardboard modeling now proves the `13.8 in` heater cylinder passes very precisely through the actual `14 in` extrusion-base opening, leaving only `0.1 in` nominal radial clearance. Product photos show a continuous black bottom skirt with no visible feet/nubs and no apparent projection beyond the jacket, but neither the manual nor available photos prove the underside or maximum skirt OD; physical-unit measurement remains the final gate. Raising the heater above the extrusion is not viable because only about `16 in` remains below the cooler. The manual's conservative pan rule calls for a minimum `15.8 in` pan diameter, so the practical camper path is a thin custom catch tray/liner beneath the entire bay, with its curb outside the `14 in` pass-through and an independent gravity drain through the bed—an explicit deviation from the manual's residential pan diameter. It is hardwired; the manual specifies a `20A` branch and `12 AWG` copper for this model. A.O. Smith E6-6C15SV remains the six-gallon fallback only if its complete installed connection geometry is physically proven. Camplux ME40B and Bosch ES4 remain four-gallon fallbacks. Hydronic/marine-calorifier, exterior-vented RV propane, and electric tankless are no longer lead candidates; Flame King YSNAZ132 remains temporary outdoor-shower-only.
- Freeze and winterization strategy (`2026-07-25` simplification): use the tank drain, the selected heater's own drain procedure, open fixtures, and a regulated pressure-side blowout connection only if needed. Do not add a permanent antifreeze pickup or three-valve heater bypass unless actual winter use later proves the extra fittings worthwhile.
...
- Current plumbing-route concept (`2026-07-25` simplified baseline): draw from a low north-end port through the existing tank shutoff, full-flow 90 only if the real turn requires it, flex, strainer, pump, discharge flex, and accumulator. Mount the purchased SHURflo/SEAFLO pack in the measured `6 in` cooler-to-battery gap on a thin vibration-isolated plate supported by extrusion; published component heights are about `4.3 in` for the pump and `4.5 in` for the accumulator, leaving only `1.7/1.5 in` theoretical margin before plate, hoses, and service sweep. Preserve strainer-bowl removal and point fittings/leak paths away from the batteries behind a nonconductive splash divider. After the accumulator, use one ordinary tee: the straight leg becomes the joint-minimized cold trunk to the sink/rear fixture, and the branch feeds the storage heater through one cold-inlet isolation valve after the tee. Heater hot runs in one insulated trunk to the faucet/shower hot sides. A second hot-out isolation valve is optional for clean heater removal; there is no cold manifold, hot manifold, or three-valve bypass in the default build. Keep the heater in parallel so cold water still works when it is off or isolated. Set the heater near its normal `120°F` range and mix locally at the faucet/shower; add a central thermostatic valve only if the selected heater will intentionally store hotter water to stretch capacity. The heater's factory T&P relief/discharge and drain requirements remain mandatory. Keep the gravity tank drain, sink gray drain, and pump-pressurized ambient-water washdown physically separate.
