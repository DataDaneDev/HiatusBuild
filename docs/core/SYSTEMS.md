...
- Solar lead path uses a continuous retractile cord in the camper-to-cab gap, with no exterior quick disconnect and no manual roof-motion step. The helix stays entirely outside; only a stationary factory straight lower tail passes through a premium fixed-camper compression gland, with structural jacket clamps outside and inside so the gland/terminals carry no spring load. After the inside clamp, exactly two accessible sealed splices transition to stationary `12/2`; no long retractile tail is needed inside. Owner-corrected geometry is approximately `24 in` overall down plus `28 in` pop-up stroke, or about `52 in` up. This withdraws the earlier `3-7 ft` span and `8.5-9 ft` cord target: research the short `1-5 ft` stock class or a custom black `12/2` around `24 in` relaxed / `60-66 in` rated overall, subject to direct clamp-center measurements and `>=150VDC`, outdoor/wet/UV/cold/coiled-ampacity and force proof. No four-to-one branch/combiner is used: three series mates leave only two string-end leads, which transition to the retractile in a compact two-polarity box. LAPP `53112025` M20 long-thread brass is the premium gland for a measured `7-13 mm` cord; the current `1203PM06` lead is `13.08-13.59 mm` and therefore requires M25 `53112035`, as does a roughly `16 mm` SOOW fallback. Open the IMO `SI32-PEL64R-2` two-pole DC-PV load-break before any de-energized matched-brand solar service connection.
