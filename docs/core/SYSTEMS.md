...
- The replacement KUS `FLS-U` under-ring/gasket/hardware is installed and the sender circuit is owner-reported wired. Verify KUS black to upper `DATA` and KUS pink/the owner's red extension to lower `GND` in one numbered Tank column, then configure/prove the `240-33 ohm` reading before closeout.
- Three separate `12 AWG / 15A` USB-PD routes are owner-reported (`1x` Desk, `2x` Galley); identify and label the third fuse-panel slot in the physical panel rather than inferring it from the old two-station map. Desk and Galley first-in-chain GFCIs are owner-reported wired on separate breakers; any downstream receptacles stay on the corresponding GFCI `LOAD` side with continuous PE and whole-chain testing.
...
- Lock the separate Wakespeed `500A/50mV` shunt into the dedicated alternator positive branch near the house board: `ALT B+ -> 2/0 -> Wakespeed shunt -> short 2/0 jumper -> F-04/Lynx Slot 3`. Purple/current-sense-high lands on the alternator side; grey/current-sense-low lands on the Lynx side; configure `Shunt at Alternator`. Because this puts both sense wires on the positive `48V` conductor, Wakespeed's `2022` Quick Start Guide requires a separate `5A` fuse in each sense lead; use holders rated above the bank maximum and mount them immediately at the shunt. The Victron SmartShunt remains the separate battery/SOC shunt.
...
- Moving-roof baseline: Starlink now uses one rugged shielded fixed-body RJ45 panel bulkhead feeding either the removable retractile roof jumper or a long outdoor shielded ground-deployment cable. Solar is not part of this interface and remains a later electrically/mechanically separate route. Full topology, current-source links, tooling notes, sizing gates, and acceptance tests live in [STARLINK_SOLAR_MOVING_UMBILICAL](../plans/STARLINK_SOLAR_MOVING_UMBILICAL.md).
...
- Starlink lead path is third-party Standard 4/4X Type-4-to-RJ45 adapters plus a shielded retractile Ethernet jumper and one Neutrik `NE8FDX-P6-W` panel feedthrough. Neutrik publishes shielded Cat6A, PoE Type 4 Class 8 / `100W`, `>1000` mating cycles, and IP65 when correctly mated/capped. The compatible roof-jumper cable-side part waits for measured cable OD/conductor construction. Preserve the complete factory Starlink cable as an unmodified direct recovery spare.
- Roof-mount lead is the TRIO Gen 3 Standard Speedmount around the dish, through-bolted at four corners into captured T-nuts on two movable extrusion crossbars. The full framed dish remains power-driver removable and keeps the factory kickstand usable. Edge protection is not branch-proof; remove/stow or ground-deploy it for tight brush.
...
- Solar lead path remains sizing-gated on final panel/string cold `Voc`, `Isc`, route length, and roof travel. igus `CF9-UL-60-04` is straight four-conductor chain cable rather than a retractile jumper and is removed from the lead moving-section BOM. Use a purpose-built two-conductor outdoor/wet/repeated-flex coil only after calculations. Open a correctly rated two-pole DC PV load-break before disconnecting matched-brand MC4 or other service connectors. IMO `SI32-PEL64R-2` is the current lead rotary service isolator (`32A`, two-pole, `600VDC`, lockable OFF, DC-PV class), but release still waits for the final array and cable selection.
