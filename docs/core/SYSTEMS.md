...
- Hot-water decision posture: electric tankless is out of scale for the current `48/3000` inverter; small tanked electric is plausible later but adds AC load-management complexity; portable propane tankless remains provisional outdoor-use only; listed indoor/RV propane is deferred until venting/combustion-air/clearance package is locked. Also keep an engine-coolant marine calorifier open as a deferred option: because the Hiatus shell is effectively a topper on the existing F-350 bed, coolant supply/return lines could be routed through the truck bed/floor and remain truck-side rather than depending on topper removability.
...
- Current water-integration order: inventory the tank's existing molded ports and mock the KUS sender/fill/vent/outlet/drain on the workbench; then perform one bare-tank in-truck dry fit before making any new penetration. The gravity fill may enter through a manufacturer-compatible top or high-side fitting—the controlling geometry is continuous downhill fill-hose routing from the exterior hatch, with a separate vent rising from the tank's highest practical point without a trapped low loop.
