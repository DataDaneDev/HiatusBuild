...
- Integrated roof-umbilical baseline: keep Starlink and PV electrically separate but manage them in one protected moving assembly at the sheltered front corner or highest viable sidewall location. One fixed-body multi-cable seal or service pod may serve both circuits; do not combine Starlink PoE/data and PV DC into one field-built electrical connector. A cable gland clamps stationary cable and must never be used as a sliding bushing.
- Preferred moving section: retain the complete factory Standard 4 X cable without a splice and carry it beside a correctly sized continuous-flex PV cable in an openable energy chain or similarly controlled straight-loop guard. Starlink's current guidance says its cable conforms to RJ45 standards but should not be modified or combined with another cable; this makes a mechanical carrier using the OEM cable lower risk than inserting an RJ45 coil. The provisional off-the-shelf pairing is an igus `2500-03-075-0` one-meter carrier (`25 x 38 mm` interior, `75 mm` bend radius) with Alpha Wire `80065` EcoFlex PUR `3C`, approximately `10 AWG`/`600V`; use the two numbered black conductors for PV and insulate/cap the unused green/yellow conductor at both ends, or quote a purpose-built two-conductor equivalent from Cicoil. The `75 mm` carrier radius clears the Alpha cable's `73.3 mm` published dynamic minimum. Final conductor, connector, fuse, disconnect, chain length, and mounting geometry still wait for actual panel/string `Voc`, `Isc`, parallel-string count, route length, cold-weather correction, and a physical popup mock.
- Trail-removable implementation: use a quick-pin upper carrier bracket so the roof end can be unplugged, capped, and stowed against the fixed body after Starlink is powered down and PV is isolated. The fixed sidewall may use a single Scanstrut `DS-H-MULTI-BLK`/`DS-MULTI-P-BLK` multi-cable seal sized against the actual preterminated connector and cable diameters. Normal-road mode remains fully connected and operational.
- Coil fallback only: the L-com `TRD815SZ` shielded Cat5e/TPU coil family is real, including `TRD815SZ-CH-6-36IN` and the roomier `TRD815SZ-CH-1-6F`, but neither is a plug-and-play Standard 4 X cable. An L-com `WPBHC6`, Neutrik etherCON TOP, or Bulgin 6000-Series interface would require an adapter or modified/custom Starlink cable, contrary to the OEM no-combine posture. Do not procure this path until the factory-cable carrier prototype fails, actual geometry is measured, and a complete Starlink load/heat/reboot/shield test is accepted. A separate `10/2 SEOW` retractile cord is a viable solar-only coil candidate if its installed voltage/current/temperature ratings pass the final array calculation.
