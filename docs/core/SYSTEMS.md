...
- Hot-water decision posture (`2026-07-26`, owner-corrected final): propane only. Selected appliance is the outdoor-use Joolca HOTTAP V2 Essentials specified by the owner at `35,000 BTU/hr` and `2.5-6 L/min` (`0.6-1.6 GPM`), with a `1.2 m / 3.9 ft` LPG hose, GASKNECT gas coupling, and `1/2 in BSP` water quick-connects. Mount it directly to a structurally backed exterior side face of the rear box with Joolca's vehicle quick plate and mounted cover; it remains outside the cylinder compartment and no articulating arm or box-side water plate is used. The camper receives one BLUE/RED two-port plate exposing Melnor QuickConnect-compatible male plugs. The supplied `1 m` and `4 m` red hoses each have female quick-connect sockets at both ends: use the `1 m` hose from camper BLUE to heater inlet if it reaches without strain, and the `4 m` hose from heater outlet either to the shower handle/head or camper RED hot return. Joolca explicitly documents Melnor QuickConnect compatibility and connection to an existing kitchen faucet. Source/fitting details and the complete inline count live in [INTERIOR_furniture_layout_and_galley](../implementation/INTERIOR_furniture_layout_and_galley.md#propane-hot-water-routing-baseline-2026-07-26-owner-corrected-final-direction).
...
- Freeze and winterization strategy (`2026-07-26` final): after every freezing-weather use, close LP, close BLUE, relieve pressure at the active outlet, close RED, disconnect and gravity-drain both Joolca hoses, then follow the HOTTAP manual's drain/storage procedure. Keep both Sea-Dog/Melnor service ports straight-through behind accessible camper ball valves so the exterior stubs drain. Clean and cap the two exposed male QuickConnect plugs. Use tank drain/open fixtures and regulated blowout only if needed; no permanent antifreeze pickup or three-valve heater bypass.
