...
- Current build phase: the electrical module, Galley/cooler support, water tank, and returned Bench extrusion are owner-reported hard-mounted; the Orion/`12V` path is operational. Driver-rear shore inlet/cable routing is installed; one enclosed `L/N/PE` splice plus dead checks remain. Battery 1 completed the corrected isolated charge cycle, Battery 2 reached absorption, and Battery 3 remains pending before rest/`<=0.1V` matching and parallel-bank commissioning. Immediate physical closeout is ICECO switch insulation/temporary mounting -> KUS termination -> pump branch -> access-dependent Galley/Desk wire pulls -> individual battery/cooler road restraint. The final walnut install has a mid-September `2026` target; full temporary tops are dropped. Roof work stays at measured Starlink/track/moving-cable design until the actual popup geometry is proven, with ground deployment accepted for the first trip.
...
- Three `48V` battery branch harnesses are physically complete and landed at the positive/negative battery-side busbars. Battery 1 completed its corrected cycle, Battery 2 reached absorption, and Battery 3 is pending; all three still require a recorded rested-voltage match within `0.1V` before they are paralleled.
...
- The ICECO appliance lead is switched and landed on `12V-02` through a `15A` fuse. Its `16 AWG` pigtail is acceptable downstream of that branch protection, but the exposed positive switch tabs must be individually insulated and the switch temporarily hard-mounted before normal powered work.
- The replacement KUS `FLS-U` under-ring/gasket/hardware is installed and its two-wire lead is routed to the Cerbo. Termination remains: KUS black to the upper `DATA` terminal and KUS pink/the owner's red extension to the lower `GND` terminal in one numbered Tank column.
...
- Moving-roof baseline: keep Starlink PoE/data and PV DC electrically separate but mount two independently strain-relieved retractile jumpers side by side. Their own spring coils store roof-down slack. Normal-road mode stays connected and operational; tight-trail mode powers/isolates each circuit, removes both jumpers, and caps the fixed ends. Full topology, current-source links, tooling notes, sizing gates, and acceptance tests live in [STARLINK_SOLAR_MOVING_UMBILICAL](../plans/STARLINK_SOLAR_MOVING_UMBILICAL.md).
...
- Starlink lead path is a bench-tested third-party Standard 4/4X Type-4-to-RJ45 adapter pair plus a shielded retractile Ethernet jumper; preserve the complete factory Starlink cable as an unmodified recovery spare. Neutrik etherCON is an optional connector-end hardening path, not protection for the coil midspan, and its cable carrier must match actual cable OD/termination construction before purchase.
- Fixed-body entry is reopened. Scanstrut `DS-H-MULTI-BLK` is withdrawn for the proposed vertical sidewall geometry; Scanstrut `TBH-4` is the measurement-gated straight through-bulkhead candidate only if the actual `10-35 mm` wall-thickness range and selected cable/connector OD prove fit. It is a seal, not a quick disconnect. Ground-deployed Starlink through the existing truck-bed service route is the accepted first-trip fallback.
- Solar lead path remains sizing-gated on final panel/string cold `Voc`, `Isc`, route length, and roof travel. igus `CF9-UL-60-04` is straight four-conductor chain cable rather than a retractile jumper and is removed from the lead moving-section BOM. Use a purpose-built two-conductor outdoor/wet/repeated-flex coil only after calculations. Open a correctly rated two-pole DC PV load-break before disconnecting matched-brand MC4 or other service connectors. IMO `SI32-PEL64R-2` is the current lead rotary service isolator (`32A`, two-pole, `600VDC`, lockable OFF, DC-PV class), but release still waits for the final array and conductor/gland checks.
