...
### Planning snapshot (base model as-of `2026-08-03`)
...
- Current build phase: the electrical module, Galley/cooler support, water tank, and returned Bench extrusion are owner-reported hard-mounted; the Orion/`12V` path is operational. Driver-rear shore inlet/cable routing is installed; one enclosed `L/N/PE` splice plus dead checks remain. Battery 1 completed the corrected isolated charge cycle, Battery 2 is near the top of bulk, and Battery 3 remains pending before rest/`<=0.1V` matching and parallel-bank commissioning. Immediate physical closeout is battery/cooler road restraint -> plywood Galley counter/sink/faucet -> Galley/Desk wire pulls while access is open -> KUS/water acceptance as parts arrive. Roof work stays at measured Starlink/track/moving-cable design until the actual popup geometry is proven.

### Physical integration snapshot (`2026-08-03`)
...
- Driver-rear shore inlet and exterior-to-module feed are installed. One accessible, enclosed, conductor/gauge-rated `L/N/PE` splice into the locked AC-input breaker path remains before dead checks and energization.
- Three `48V` battery branch harnesses are physically complete and landed at the positive/negative battery-side busbars. Battery 1 is charged, Battery 2 is finishing, and Battery 3 is pending; all three still require a recorded rested-voltage match within `0.1V` before they are paralleled.
...
- Integrated roof-umbilical baseline: keep Starlink and PV electrically separate but manage them in one protected moving assembly at the sheltered front-center first or highest viable sidewall second. One fixed-body multi-cable seal or service pod may serve both circuits; do not combine Starlink PoE/data and PV DC into one field-built electrical connector. A cable gland clamps stationary cable and must never be used as a sliding bushing.
- Preferred moving section: retain the complete factory Standard 4 X cable without a splice and carry it beside a separately strain-relieved continuous-flex PV cable in a broad vertical S-loop captured by a light aluminum/UHMW guide, sleeve, or pouch. Starlink's current guidance discourages modifying or combining its cable, so the unbroken OEM path remains the lowest-risk architecture. The provisional moving-PV cable class is approximately `10 AWG`, fine-strand, outdoor/UV-capable continuous-flex cable such as igus `CF9-UL-60-04` (`4C`, `1000V`; use two conductors and insulate/cap the unused pair) or a purpose-built two-conductor Cicoil equivalent. Final conductor, connector, OCP, disconnect, free-loop length, and mounting geometry still wait for actual panel/string `Voc`, `Isc`, parallel-string count, route length, cold-weather correction, and a physical popup mock.
- Energy-chain upgrade only if the guided-loop mock rubs, kinks, hangs, or packs poorly. Size an outdoor-capable igus `E2.21`-class chain only after measuring both cable outside diameters, factory connector envelope, minimum dynamic bend radii, full roof travel, and roof-down storage volume; do not pre-buy a one-meter chain on the `36 in` planning figure alone.
- Trail-removable implementation: power down Starlink and open a DC-rated PV load-break before unplugging any roof-side service connector. Stow the OEM Starlink cable with the terminal/cable cartridge or in the front guard; cap the isolated PV service connector. The fixed body may use one Scanstrut `DS-H-MULTI-BLK`/Blue Sea `3115` multi-cable seal or a partitioned service pod sized against the actual preterminated connector and cable diameters. Normal-road mode remains fully connected and operational. MC4, Anderson, or Bulgin connectors are service disconnects after current is stopped, not normal load-break devices; use a correctly sized MidNite `MNEPV`-class breaker/disconnect or equivalent.
- Coil fallback only: the L-com `TRD815SZ-CH-6-36IN` shielded Cat5e/TPU coil is a real `6 in`-to-`36 in` geometry candidate, but it is not a plug-and-play Standard 4 X cable and would operate at its full rated extension if the actual anchor travel is `36 in`. The historical L-com `WPBHC6` manufacturer product path now returns `404`; treat it as unavailable rather than a current buy. Any Bulgin 6000-Series Cat6A conversion still requires an adapter or modified/custom Starlink cable and complete load/heat/reboot/shield/weather validation. A solar-only retractile lead remains a fallback if its installed voltage/current/temperature ratings pass the final array calculation.
...
- Installed pump/accumulator state (`2026-07-26` owner report): the purchased SHURflo `4008-101-A65` `3.0 GPM` pump, SEAFLO inline accumulator, tank, and interior PEX distribution operate under pressure. Use approximately `37 PSI` accumulator precharge with the water side fully depressurized, based on the pump's nominal `40 PSI` cut-in; the accumulator is not a regulator and the static system may begin near the pump's `55 PSI` cutoff.
...
- Interior plumbing state (`2026-08-02` owner report): all three passenger-rear camper pass-throughs—the gravity-fill hatch, direct BLUE cold-out, and RED hot-return—are physically installed; BLUE/RED are functioning. The BLUE source has the required accessible ball valve; the RED leg is intentionally unvalved because it is only the removable heater return to the sink hot inlet. The projecting HQMPC-style male QDs physically fit, and the owner-designed clicked-on travel caps latch and release correctly. Final water acceptance still requires the KUS sender repair, fill/vent hose closeout, sink termination, and a dry pressure/dwell inspection after all disturbed joints are complete.
- Sender-mount failure (`2026-07-27` owner report): the first KUS SSS/SSL plus `FLS-U` under-ring installation damaged at least one backing-ring thread while a screw was being backed out. One screw is now seized or free-spinning without axial movement/clamp load, and water pours through the sender gasket when the tank is inverted. The now-mounted tank is not water-serviceable in this state. Preserve top access, remove the failed hardware, replace the `FLS-U` ring/main gasket/matched long screws, inspect the tank sealing land and sender flange, then retest before first fill or water service. The short screws with bonded metal/rubber sealing washers belong to KUS's separate gasket-mounting kit; the official `FLS-U` manual depicts its longer under-ring screws without transplanted loose rubber pieces. Loss of clamp load—not omission of those donor washers—is the controlling failure.
...
- Current water-integration order: repair and leak-prove the cut KUS sender opening first; then finish inventorying/measuring the eight reported end ports, freeze the fill/vent/outlet map, and prove installed hose/service geometry before any remaining tank or camper penetration. Current default is an upper large end port for gravity fill and the highest suitable upper small end port for the unrestricted vent/overflow. The controlling geometry is continuous downhill fill-hose routing from the exterior hatch plus continuous vent rise without a trapped low loop; neither function requires another top-surface port if the selected end ports remain in the tank airspace.
...
- Exterior-cut dependency update (`2026-08-02`): the driver-rear shore inlet and three passenger-rear water pass-throughs are installed. Remaining work is internal/system closeout—shore splice/dead checks, fill/vent hose completion, KUS repair, sink termination, and final wet/AC acceptance—not additional convenience-driven shell cuts.
...
- Current physical state (`2026-08-02` owner report): electrical, Galley/cooler support, water tank, and the Bench extrusion bridge are hard-mounted and the integrated structure is very stiff. Build the battery restraint as a low extrusion perimeter/divider cage tied to anchored extrusion or through-bolted structural plywood, with individual over-case straps and no strap contact at terminals/displays. Restrain the ICECO at its foot-level tie points with a low-stretch cam strap to the support extrusion plus a lateral hard stop and soft wall bumper; do not crush the cooler case with ratchet tension.
