...
- Owner-reported `12V` checkpoint (`2026-08-01`): the Orion is charging the buffer battery and the camper fan/lights operate from the shared `12V` junction. The existing multimeter reported an implausible `18V` at the `12V` terminals while VictronConnect reported about `13.5V`; treat `18V` as a stop-and-verify condition rather than assuming either instrument is correct. Disable the Orion, replace the meter battery or use a second known-good meter, verify the battery directly at its posts with the Orion off, then compare battery-post, Orion-output, and VictronConnect values after re-enabling. Do not close commissioning on this discrepancy until independent voltage agrees.
...
- Interior plumbing state (`2026-08-01` owner report): the tank/pump/accumulator pack and most interior PEX are assembled, and both direct BLUE cold-out and RED hot-return pass-throughs are installed and functioning. The BLUE source has the required accessible ball valve; the RED leg is intentionally unvalved because it is only the removable heater return to the sink hot inlet. The projecting HQMPC-style male QDs physically fit, and the owner-designed clicked-on travel caps latch and release correctly. Final water acceptance still requires a dry pressure/dwell inspection after the sink, sender, fill/vent, and all disturbed joints are complete.
