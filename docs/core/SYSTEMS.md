...
### Planning snapshot (base model as-of `2026-08-27`)
...
- AC protection chain is purchased/locked for Phase 1 (`shore source/adapters -> portable EMS -> shore cord -> L5-30 inlet -> single 6-way AC DIN enclosure -> 30A AC-in breaker -> MultiPlus -> 30A AC-out main -> two 20A GFCI branches`). AC-in/MultiPlus charging passed a limited-current live test. Four duplex receptacles are now wired—one GFCI origin plus one `LOAD`-protected downstream device on each branch—but AC-out acceptance remains open for box/clamp/cover, `LINE/LOAD`, PE, polarity, label, and trip/reset proof.
- Working `12V` endpoints now include the switched Starlink Standard 4 X conversion branch through a `20A` blade fuse, switched fridge and pump, one separately fused driver-side `100W` charging outlet, and two separately fused passenger-side `65W` charging outlets. Replace the hanging/taped switch bundle with a labeled removable service face before normal powered use.
- All four Renogy `175W` panels have arrived; one-panel physical test-fit plus measured layout indicates compatibility with Starlink/MaxxAir and near-total roof use. Keep the `4S1P`/Victron `150/45` architecture. Direct bonding is silicone/product/preparation/cure gated; the PV retractile cord is ordered and remains received-property/full-motion gated.
- The LF Bros diesel heater, fuel storage/line, and rear driver-side storage module are the main coupled installation gate. The mid-September walnut-routing appointment is the next fixed milestone; bring the actual sink and soap dispenser and prove the full drain/support envelope before cutting.

### Physical integration snapshot (`2026-08-27`)
- The permanent floor, electrical/Galley/Bench structure, water tank, and primary utility routes are installed. Remaining mobile restraint is localized to batteries, ICECO, air fryer, bench lid, monitor/laptop assembly, and final terminal/cable protection.
- Driver-rear shore inlet and exterior-to-module feed are installed. One accessible, enclosed, conductor/gauge-rated `L/N/PE` splice into the locked AC-input breaker path remains before permanent-path use.
- Three `48V` battery branch harnesses are physically complete and landed. Existing isolated-charge/rested-voltage/matching and positive-restraint gates remain open unless separately logged as passed.
...
- Starlink, fridge, pump, and all three DC charging outlets are owner-reported wired and switched/branched as applicable. Confirm exact physical fuse slots, fuse values other than Starlink's confirmed `20A`, polarity, loaded drop, labels, and terminal insulation rather than inferring completion from operation alone.
- The replacement KUS `FLS-U` under-ring/gasket/hardware is installed and its sender circuit is owner-reported wired. Verify KUS black to upper `DATA` and KUS pink/the owner's red extension to lower `GND` in one numbered Tank column, then configure/prove the `240-33 ohm` reading before closeout.
...
- Overnight Battery 2 checkpoint (`2026-08-02/03`): still in bulk in the upper `54V` range and progressing normally toward the `56.8V` absorption target; this is not an abnormal state by itself. Battery 3 remains the final isolated-charge member.
- Owner-reported `12V` checkpoint (`2026-08-02`): the Orion charges the buffer battery and the camper `12V` system is operating correctly. The prior implausible handheld-meter discrepancy is no longer reported as an active behavior; retain the normal rule that any independently confirmed bus voltage above the configured `14.20V` absorption target is a stop condition.
...
- PV moving power is the ordered exterior retractile cord in the front camper-to-cab gap. Preserve the three series mates and route only the two free string ends through a compact two-pole transition, continuous `12 AWG` two-conductor cord, fixed two-pole disconnect, then short `12 AWG` two-conductor run to the MPPT. `12 AWG` is the conductor size; this is not a `12V` circuit.
- The fixed-camper compression gland is planned just above the installed Starlink entry behind the cab. The owner accepts residual branch/snatch exposure, but exterior adhesive zip-tie mounts may only guide the run: structural redundant clamps/guards must carry coil spring, motion, and highway loads. Verify received cord conductors/OD/rating/tangents/force and full `28 in` roof stroke before drilling.
...
- Solar lead path uses the ordered continuous retractile cord in the camper-to-cab gap, with no exterior quick disconnect and no manual roof-motion step. The fixed-camper compression gland is planned just above the installed Starlink entry behind the cab; the owner accepts residual branch/snatch exposure. Keep the helix outside and pass only a stationary lower tail through the gland, with structural redundant clamps outside and inside so the gland, adhesive guides, and terminals carry no spring/highway load. After the inside clamp, exactly two accessible sealed splices transition to stationary `12/2`. Geometry is approximately `24 in` overall down plus `28 in` roof stroke, or `52 in` up. On receipt, verify the exact cord's two-conductor need, OD/gland fit, `>=150VDC`, wet/UV/cold/coiled-ampacity, tangent geometry, and force curve before drilling. Preserve `4S1P`: three series mates leave only two string-end leads, which transition through a compact two-polarity box, the cord, the IMO `SI32-PEL64R-2` two-pole load-break, and the MPPT. No four-to-one branch/combiner is used.

## Plumbing (if included in phase 1)
- Near-term baseline: complete the cold-water galley and one camper-side two-port propane hot-water service interface together. Build around tank, pump, accumulator, faucet/sink, BLUE pressurized cold-out, RED hot-return, drain/graywater path, and accessible service shutoffs. The HOTTAP itself mounts directly to the outside face of the rear box; there is no second water plate on the box.
...
- Owner-confirmed pre-cut tank port geometry (`2026-07-16`): four molded ports on each end, with two visibly large and two smaller ports per end; no original obvious/open top port. Exact model and thread sizes remain unconfirmed. A new `60 mm / 2-3/8 in` sender opening was subsequently cut; fill/vent assignments still require physical port/thread verification.
...
- Prior plumbing function evidence (`2026-07-26` owner report): before final installed closeout, the purchased SHURflo `4008-101-A65` `3.0 GPM` pump, SEAFLO inline accumulator, tank, and interior PEX operated under pressure. Preserve that dated evidence, but do not treat it as the `2026-08-27` installed pump-branch acceptance; repeat loaded electrical and dry-compartment pressure/dwell tests after all final joints/terminations are complete. Use approximately `37 PSI` accumulator precharge with the water side fully depressurized, based on the pump's nominal `40 PSI` cut-in; the accumulator is not a regulator and static pressure may begin near the `55 PSI` cutoff.
...
- Current water-integration order: terminate/configure the installed KUS sender, finish fill/vent and sink terminations, then perform one complete dry pressure/dwell leak proof while the sender remains visible. The controlling fill geometry is continuous downhill fill-hose routing from the exterior hatch plus continuous vent rise without a trapped low loop.
...
- Exterior-cut dependency update (`2026-08-03`): the driver-rear shore inlet and three passenger-rear water pass-throughs are installed. Remaining work is internal/system closeout—shore splice/dead checks, fill/vent hose completion, KUS termination/leak proof, sink termination, and final wet/AC acceptance—not additional convenience-driven shell cuts.

## Cabinetry and structure
- Interior furniture/layout draft owner: [INTERIOR_furniture_layout_and_galley](../implementation/INTERIOR_furniture_layout_and_galley.md). Recommended planning direction is the office-first hybrid: passenger-side lofted Iceco/fridge on an extrusion exoskeleton, pump/accumulator below the fridge next to the `36 gal` wheel-well tank, adjacent separated battery bench, driver-side electrical closet/workstation/DC shelf, diesel heater low in the driver-side utility zone, and clear center aisle/cabover movement.
...
- Current physical state (`2026-08-27` owner report): the main electrical/Galley/Bench structure is hard-mounted; the rear driver-side storage module, bench lid, desk/monitor hardware, air-fryer home, and final skins remain. Coordinate the rear module with the diesel tank/pump/fuel route before installing either. Preserve the roughly `24 in` desk until the real module, chair, knees, and aisle are mocked; shrink toward `20 in` only if measured seated/entry clearance requires it. A folding/stowed chair is a valid space-saving option.
- Travel restraint baseline: each battery and the ICECO retain their positive-capture requirements. The air fryer needs shelf lips/hard stops plus a positive strap/latch; pressure-sensitive tape alone is not the primary road restraint. Store the basket/rack/pan so they cannot chatter or become projectiles; any soft separator placed inside must be conspicuous and removed before heating. The monitor/laptop arms need a padded hard-stop cradle contacting safe structure/bezel/VESA areas, with restraint that unloads the arms and does not press on an LCD. Size bench gas struts from measured lid mass/center of gravity and hinge/bracket geometry; use symmetric geometry plus a separate positive closed latch and mechanical open stop.
...
- Purchased heater baseline: LF Bros `5 kW / 12V / 10 L Split Pro`, with supplied tank, external metering pump/filter, exhaust/muffler, combustion intake, T4S hardwired controller, wireless remote, harness, and mounting hardware. Detailed fit/cut owner: [INTERIOR_driver_side_workstation](../implementation/INTERIOR_driver_side_workstation.md#diesel-heater-base-zone).
- Heater install posture (`2026-08-27` owner report): the heater location is above/adjacent to the truck fuel-tank envelope and the contemplated exhaust route is nearby. Do not release that layout from heat shield alone. Preserve at least `10 cm / 4 in` at the rear cabin-air inlet and an unrestricted warm-air outlet; use a sealed metal floor turret/plate directly below the heater; and physically survey the full underside for the truck fuel tank, fuel/EVAP/brake lines, wiring, undercoating, structural members, plastics, and service access before drilling.
...
- Supplied tank/pump baseline: fuel order is `tank -> filter -> pump -> heater`; the pump outlet points toward the heater and upward about `45 degrees`, with pump-to-heater distance within `2 m`. Use a proper floor bulkhead/grommet, protected flexible fuel line, structural pump mount, strain relief, and anti-chatter support. The pump belongs near the tank on the exterior side when practical, not loose beneath the living floor.
- Fuel-storage posture: coordinate tank, pump, floor entry, heater, and rear driver-side storage-module geometry before installing the module. Do not place the supplied open-vent plastic tank/fill/filter/pump in ordinary living, battery, electrical, or general-storage space. Preferred direction is exterior mounting; an interior location requires sealed, externally vented spill containment isolated from occupied/electrical storage. The supplied `10 L` tank and pump cover commissioning hardware if a compliant containment/location is proven.
