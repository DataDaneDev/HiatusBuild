...
- Placement baseline under evaluation: use the existing front truck-bed opening for the fixed interior-to-lower-front cable route, then place the moving-roof transition on the sheltered front face rather than the camper side where branches can catch it. A cable gland must clamp a stationary cable; do not design the cable to slide through a roof gland during popup travel.
- Moving-section preference: first prototype an always-connected protected straight service loop in a sleeve/guard. If a coil is needed, use only a full-eight-conductor shielded industrial Ethernet assembly with proven Starlink PoE behavior, then keep its RJ45 ends inside sealed boxes or use an equivalently rated rugged connector system. The verified current product family is L-com `TRD815SZ` high-flex shielded Cat5e/TPU coil cord; `TRD815SZ-CH-6-36IN` is the compact `6 in`-to-`36 in` expansion candidate. Cat5e bandwidth is adequate for this link, but the `26 AWG` moving segment is not accepted for Starlink power until factory-cable connector compatibility, shield continuity, loaded voltage drop/heat, weather sealing, roof-down envelope, required extension, and repeated popup cycles are bench/geometry-proven.
- Removable fallback: use a capped, shielded, weather-rated bulkhead system rather than a bare exterior RJ45. L-com `WPBHC6` is a current Cat6/IP67 bulkhead candidate when paired with its compatible sealed cable-side housing; Bulgin `6000 Series Buccaneer` Cat6A provides a more rugged IP68 quick-turn connector family. Either still needs an actual Starlink load test and should be located on the protected lower-front face, not on an exposed roof edge.
