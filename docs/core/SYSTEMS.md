...
- Outside routing baseline: exhaust and muffler stay completely outside, fall slightly for condensate, use broad sweeps rather than a hard `90-degree` turn, and remain separated/shielded from vehicle fuel systems, wiring, plastic, undercoating, body openings, and the combustion intake. For the supplied LF Bros two-stub oval muffler, the official connection video shows the corrugated exhaust connected to one stub and the other bare stub serving as the terminal outlet; do not cut the long exhaust pipe in half merely to add a downstream tail. Hard-mount the muffler by its bracket, point any condensate drain down, and follow an actual flow arrow if present. Heat shield is supplemental to real clearance, noncombustible standoffs/guards, and a route that cannot contact or heat the truck fuel tank. Intake and exhaust must not terminate in the same direction or where exhaust can be recirculated.
...
- Fuel-system field-fit update (`2026-09-06` owner report, superseding the purchased routing description where different): the external tank package is largely assembled and the gravity-fill pass-through is near completion; the vent line is physically complete but not yet acceptance-tested. The remote fill still routes through a lateral 45-degree hose dogleg, an owner-described stainless coupler, straight hose, and a 90-degree joiner at the Speedway flange. Owner cut off the 90's tank-facing bead/barb end for clearance and plans a roughly `4 in` hose coupling; a slight hose kink remains. These modifications are recorded, not approved: actual bend opening, straight clamp lands, deburring/cleanliness, hose retention after bead removal, and relative-movement clearance need inspection before fuel. The straight-through fuel filter is now mounted vertically, and the pump is laid out with its outlet toward the heater and about `45 degrees` upward; the small fuel line is not yet run. Bed-wall filler mounting is planned as supplied gasket plus through-bolts without extra bedding at that open-air interface. The direct EVIL ENERGY `10AN -> 1/8 NPT` fitting is reordered; turret sealing, pump/heater plumbing, fused wiring, initial leak test, and first burn remain pending. Current checks: [live build checklist](../plans/LIVE_BUILD_CHECKLIST.md#13-comms-lighting-and-comfort-systems).
