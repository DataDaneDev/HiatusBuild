...
- Solar lead path uses a continuous retractile cord in the camper-to-cab gap, with no exterior quick disconnect and no manual roof-motion step. The helix stays entirely outside; only a stationary factory straight lower tail passes through a fixed-camper compression gland, with structural jacket clamps outside and inside so the gland/terminals carry no spring load. Preferred moving conductor is `12 AWG x 2` after final route/derating proof. Coil Cable Specialist `1203PU06` (`12/3`, `600V`, `25A`, `0.400 in` cable / `1.500 in` coil, `$61.25` shortest stock screen) is the compact prototype lead only after written `>=150VDC`, coiled-ampacity, tangent, wet/outdoor, and cold confirmation; Autac is the custom `12/2` fallback. The protected gap still needs full-stroke and cab/bed-articulation proof. Open the IMO `SI32-PEL64R-2` two-pole DC-PV load-break before any de-energized matched-brand MC4 service disconnect.
