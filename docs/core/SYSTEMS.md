...
- Current physical state (`2026-09-01` owner report): the main electrical/Galley/Bench structure is hard-mounted; the rear driver-side storage module, bench lid, desk/monitor hardware, air-fryer home, and final skins remain. Coordinate the rear module with the diesel tank/pump/fuel route before installing either. The FLEXISPOT Foldex folding ergonomic office chair is purchased but not yet physically accepted. Preserve the roughly `24 in` desk until the real module, purchased chair, knees, and aisle are mocked; shrink toward `20 in` only if measured seated/entry clearance requires it.
...
- Purchased heater baseline: LF Bros `5 kW / 12V / 10 L Split Pro`, with external metering pump/filter, exhaust/muffler, combustion intake, T4S hardwired controller, wireless remote, harness, mounting hardware, and the supplied plastic tank now reassigned to graywater. The purchased diesel reservoir is the EVIL ENERGY `10 gal` aluminum tank/feed/vent package in BOM row `65`. The floor interface is the purchased CaLeQi stainless `60 mm / 2.37 in` turret in BOM row `362`, nominally `200 x 180 mm` with a `125 mm` circular skirt. Detailed fit/cut owner: [INTERIOR_driver_side_workstation](../implementation/INTERIOR_driver_side_workstation.md#diesel-heater-integration-gate).
