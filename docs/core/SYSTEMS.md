...
- Hot-water decision posture (`2026-07-26`, owner-corrected): propane only. The purchased core package is one outdoor-use Joolca HOTTAP V2 Essentials, one Quick-Release HOTTAP Bracket, and one HOTTAP V2 Mount Cover. Mount it directly to a structurally backed exterior side face of the rear box with the purchased bracket and cover; it remains outside the cylinder compartment and no articulating arm or box-side water plate is used. The final camper-port hardware is reopened pending the actual HOTTAP hose test. Lead prototype is one compact `304 stainless` bulkhead per port (`1/2 in FNPT` rear, `3/4 in MGHT` front) with a directly threaded all-metal male tap adapter that physically proves compatible with the Joolca female socket—GARDENA `39004-G` is first candidate. Each port stays behind an accessible valve; this removes the QD3 key, `1/4 in` reducer, rear `3/8 in` hose-barb transition, and second QD ecosystem. QD3 remains premium fallback only if a clean direct-profile fitting fails. Use the supplied `1 m` hose as the short BLUE-to-heater feed if reach/routing pass and the `4 m` hose from heater outlet either to the shower handle/head or camper RED hot return. Joolca's current manual lists separate `1 m` and `4 m` hoses and states that HOTTAP is recreational wash hardware, not a drinking/sanitary-water appliance. Source/fitting details and the complete inline count live in [INTERIOR_furniture_layout_and_galley](../implementation/INTERIOR_furniture_layout_and_galley.md#propane-hot-water-routing-baseline-2026-07-26-owner-corrected-final-direction).
- Rear propane package: the Flame King `YSN10LB-ALM` was purchased `2026-07-26`; it is `9 x 9 x 19.25 in`, `9.6 lb` empty, and about `19.6 lb` filled. It travels upright, valve closed, and LP-disconnected in the owner-selected ventilated bumper box. The purchased generic two-pack of `4-12 in` wall brackets and `59 in` straps is mockup hardware only: no mobile/dynamic load rating is established and the supplied expansion screws are not camper structural fasteners. Capture the cylinder foot ring in a rigid bolted/welded cradle and use a rated body strap to proven structural backing as the primary lateral restraint; thin EPDM pads and a rubber strap are anti-chafe/anti-rattle measures, not the only load path. The reported `19.5 in` shelf opening leaves only `0.25 in` nominal vertical clearance, so prove the real cylinder, shelf, valve, regulator-connector sweep, and pad thickness before drilling. Provide permanent low drainage ventilation plus a fixed high vent and keep the valve/collar accessible.
...
- Keep the purchased Flame King `YSN10LB-ALM` upright in a rigid foot capture plus rated body restraint to proven structure, with valve/QCC1 access, valve protection, thin anti-chafe pads, permanent unobstructed low/high box vents, and an open drain path. Do not treat the purchased generic wall brackets, straps, or expansion screws as a verified mobile load path.
- Keep the cylinder shutoff accessible without tools. Protect the regulator/hose sweep and service-hatch edge from pinch points, road debris, abrasion, and exhaust heat.
- The selected LP path remains `cylinder -> supplied QCC1 regulator/hose -> deployed HOTTAP` entirely within the rear-box/outdoor package; no propane line or union passes into the camper. The purchased CALPOSE inline pressure gauge is not part of that default path because it adds LP joints and does not reliably measure remaining liquid fuel; only evaluate it if exact manual compatibility and a leak test are satisfied.
