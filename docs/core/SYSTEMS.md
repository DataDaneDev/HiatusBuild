...
- AC protection chain is purchased/locked for Phase 1 (`shore source/adapters -> portable EMS -> shore cord -> L5-30 inlet -> single 6-way AC DIN enclosure -> 30A AC-in breaker -> MultiPlus -> 30A AC-out main -> two 20A GFCI branches`). All four duplex receptacle boxes and the remaining `120V` work are enclosed and in regular owner-reported use. Physical enclosure completion is closed; retain explicit `LINE/LOAD`, downstream GFCI trip, PE/chassis/shell bonding, polarity, neutral-isolation, and labeling proof.
...
- Driver-rear shore inlet, AC feed, enclosures, and all four outlet boxes are installed and in regular use. The remaining AC safety item is not another box: it is the MultiPlus external PE-to-truck-chassis bond, separate conductive-shell bond, and explicit PE/neutral/polarity/GFCI proof.
