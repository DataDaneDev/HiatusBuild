...
- Purchased fuel-vent/cap topology: leave the unused lower outlet capped. Listing imagery indicates the filler cap and two lower `-10AN` caps are included; when one lower cap is freed by the feed connection, move it to one upper `-10AN` port and use the other upper port for `-10AN female / 5/16 in barb -> 5/16 in NBR hose -> remote rollover vent valve`. Verify the delivered port count/caps and determine whether the filler cap is sealed or vented. The remote rollover valve must be the only atmospheric vent path unless the cap vent has equivalent rollover shutoff. Mount the valve upright, above normal fuel level, and terminate it outside the occupied/storage envelope away from ignition and body openings. Leak-check a small outdoor test fill before installation/service.
- Fuel-storage posture: the `10 gal` tank is approximately `86 lb` full and must be positively restrained as a vehicle fuel load. Exterior mounting remains preferred; any interior location requires liquid-tight, externally vented containment isolated from occupied, electrical, and general-storage space. Coordinate tank restraint, filler access, vent, shutoff, filter/pump, floor entry, heater, and rear driver-side storage-module geometry before installing the module.
