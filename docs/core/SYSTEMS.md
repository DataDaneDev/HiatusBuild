...
- Fuel-system field-fit update (`2026-09-05` owner report, superseding the purchased routing description where different): vent line physically complete, not yet acceptance-tested; remote fill routes through a lateral 45-degree hose dogleg, an owner-described stainless coupler, straight hose, and a 90-degree joiner at the Speedway flange. Owner cut off the 90's tank-facing bead/barb end for clearance and plans a roughly `4 in` hose coupling; a slight hose kink remains. These modifications are recorded, not approved: actual bend opening, straight clamp lands, deburring/cleanliness, hose retention after bead removal, and relative-movement clearance need inspection before fuel. Bed-wall filler mounting is planned as supplied gasket plus through-bolts without extra bedding at that open-air interface. The direct EVIL ENERGY `10AN -> 1/8 NPT` fitting is reordered; pump/heater plumbing, fused wiring, initial leak test, and first burn remain pending. Current checks: [live build checklist](../plans/LIVE_BUILD_CHECKLIST.md#13-comms-lighting-and-comfort-systems).
