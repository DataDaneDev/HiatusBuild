...
- Current plumbing-route concept: draw from a low north-end port through a tank shutoff and reinforced flex to the north strainer/pump; start fixed `1/2 in` PEX after the pump outlet flex/accumulator; branch the cold manifold to the middle sink and south heater/washdown interfaces; return future hot water from the south heater interface to the middle sink, with an optional short south shower branch. Keep the gravity tank drain, sink gray drain, and pump-pressurized ambient-water washdown physically separate.
