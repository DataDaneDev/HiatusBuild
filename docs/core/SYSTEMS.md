...
- Interior plumbing state (`2026-07-26` owner report): the tank/pump/accumulator pack and most interior PEX are assembled. The sink is not installed; hot and cold runs terminate at accessible ball valves and final `90` fittings near the sink/rear-service areas. The first pressure/function test showed no visible leakage, but intentional open-ball-valve discharge wet the area enough that it was not a conclusive dry leak test. Repeat after the remaining fittings are installed with all surfaces dry, paper indicators at joints, and no unexplained pump cycling.
- Sender-mount failure (`2026-07-27` owner report): the first KUS SSS/SSL plus `FLS-U` under-ring installation damaged at least one backing-ring thread while a screw was being backed out. One screw is now seized or free-spinning without axial movement/clamp load, and water pours through the sender gasket when the tank is inverted. The tank is not serviceable in this state. Remove the failed hardware, replace the `FLS-U` ring/main gasket/matched long screws, inspect the tank sealing land and sender flange, then retest before permanent tank installation. The short screws with bonded metal/rubber sealing washers belong to KUS's separate gasket-mounting kit; the official `FLS-U` manual depicts its longer under-ring screws without transplanted loose rubber pieces. Loss of clamp load—not omission of those donor washers—is the controlling failure.
- Hot-water decision posture (`2026-07-26`, owner-corrected): propane only. The purchased core package is one outdoor-use Joolca HOTTAP V2 Essentials, one Quick-Release HOTTAP Bracket, and one HOTTAP V2 Mount Cover. Mount it directly to a structurally backed exterior side face of the rear box with the purchased bracket and cover; it remains outside the cylinder compartment and no articulating arm or box-side water plate is used. The final camper-port hardware is reopened pending the actual HOTTAP hose test. Lead prototype is one compact `304 stainless` bulkhead per port (`1/2 in FNPT` rear, `3/4 in MGHT` front) with a directly threaded metal male tap adapter that physically proves compatible with the Joolca female socket. Melnor `15409` is unavailable and remains only the visual/profile reference despite Joolca's documented Melnor QuickConnect compatibility. The acquired HQMPC two-set ASIN `B07FHXLKH5` supplies two visually near-identical female-GHT/male-QD adapters and is the returnable first metal test; exact latch dimensions remain unverified. The RAINPAL bulkhead two-pack and GHT travel caps are also acquired. GARDENA `39004-G` remains secondary. Each port stays behind an accessible valve, removing the QD3 key, `1/4 in` reducer, rear `3/8 in` hose-barb transition, and second QD ecosystem. QD3 remains premium fallback only if a clean direct-profile fitting fails. HOTTAP Essentials supplies one `5 m` shower assembly split into `4 m` and `1 m` red female/female sections; preserve both for their intended hot-side duties and prefer a separate roughly `30 in` BLUE-to-heater leader with Melnor `9MQC`/`8MQC` female sockets after fit testing. The `1 m` red section remains a valid no-purchase cold-feed trial, not the preferred final hose. Joolca states that HOTTAP is recreational wash hardware, not a drinking/sanitary-water appliance. Source/fitting details and the complete inline count live in [INTERIOR_furniture_layout_and_galley](../implementation/INTERIOR_furniture_layout_and_galley.md#propane-hot-water-routing-baseline-2026-07-26-owner-corrected-final-direction).
...
- Freeze and winterization strategy (`2026-07-26`): after every freezing-weather use, close LP, close the active BLUE/RED service valves, relieve pressure at the faucet/shower, disconnect and gravity-drain both Joolca hoses, then follow the HOTTAP manual's drain/storage procedure. Arrange the short PEX-to-bulkhead terminations without trapped low points, clean the male exterior plugs, and fit simple measured stretch caps for travel. Use the pump and fully open BLUE outlet for normal tank emptying; the low south boss stays unopened or receives only a low-profile plug because an exposed valve would occupy the entry footpath. A removable upstream suction-flex connection remains the dead-pump gravity fallback. Emptying the tank does not winterize the strainer, pump, accumulator, fixture cartridges, QDs, or low loops; use drain/disconnect steps and regulated blowout only if needed.
