...
- Context: Owner selected the HOTTAP V2, Joolca vehicle quick plate, and mounted cover specifically to keep the heater outside the propane compartment, protect it from road dust, and recover box storage. Research confirmed that Joolca's supplied `1 m` and `4 m` red hoses each use female quick-connect sockets at both ends; the shower handle/head disconnects from the long hose, and Joolca explicitly documents Melnor QuickConnect compatibility plus existing-faucet use.
...
- ID: D-058
- Date: 2026-07-26
- Decision: Make a direct Joolca/GARDENA-profile metal wall plug on a compact stainless bulkhead the lead camper-port prototype; retain QD3 only as the premium automatic-shutoff fallback.
...
- Result: lead prototype per port is `1/2 in PEX-B -> accessible valve -> shortest supported PEX-B -> purchased UP120A5 -> compact 304-stainless bulkhead (1/2 in FNPT rear / 3/4 in MGHT front) -> physically verified all-metal Joolca-compatible tap adapter`. RAINPAL `SSBF020` is the first bulkhead candidate (`27 mm / 1-1/16 in` opening, `36 mm` flange, maximum `6 mm` panel when the outer GHT is used). GARDENA `39004-G` / UPC `066283390047` is the first metal plug candidate. A direct `PEX-B x MNPT` combination valve such as NSF/ANSI 61-G Legend `T-805MNL` may replace the separate valve/short PEX/UP120 stack only if the physical depth mockup earns the additional purchase. The manual-valve routine is explicit: connect closed, open active port(s), close and relieve before disconnecting. QD3 remains fallback if the real-hose test cannot produce a clean metal direct-profile plug or if one-click automatic shutoff later proves worth the cost and key.
- Follow-up: when the HOTTAP arrives, take one hose and the included double-male hose coupler to the store. First use plastic Melnor `2MQC`/starter hardware as a profile control, then test GARDENA `39004-G`, Melnor brass `15409`, Morvat, and any stocked metal Original-GARDENA-profile plug for full latch depth, O-ring sealing, low play, and clean release. Separately test the Joolca double-male coupler in a Melnor `4MQC` water-stop socket as the inexpensive automatic-shutoff fallback. Bench-pressure-test the chosen stack, verify cap fit and valve access, and prove the bulkhead's panel/backer thickness before any camper cut. Use the supplied `1 m` hose as the first cold-feed trial; only shorten or build a roughly `25-29 in` hose after the mounted geometry is measured.
...
- Lock shore inlet and water-fill/vent exterior locations only after their full inside routes/endpoints, backing, service access, strain relief/hose support, and sealing details are physically proven. The purchased Safoner `3.5 in` cable hatch is alternate hardware only; decide use, reserve, or return without silently replacing the locked L5-30 inlet in BOM row `107`.
...
- Propane hot-water direction and rear-box package are locked; physical geometry remains open. Receive and dry-fit the purchased Flame King `YSN10LB-ALM` in the reported `19.5 in` shelf opening and prove valve/QCC1 access, rigid foot capture, approved body restraint, thin EPDM anti-chafe pads, permanent low/high vents, and no blocked drain path. The purchased generic `4-12 in` wall-bracket/`59 in` strap two-pack is a mockup candidate only: no dynamic rating is established and its expansion screws are not camper fasteners. Leave the purchased CALPOSE gauge out of the default LP path unless manual compatibility and leak testing justify the added joints. Direct-mount the purchased Quick-Release HOTTAP Bracket to structural backing on the exterior box side, fit the purchased HOTTAP V2 Mount Cover, and prove operating clearances, exhaust direction, regulator/service-hatch sweep, supplied `1 m`/`4 m` hose reach, faucet hot-only flow above `0.6 GPM`, complete freeze draining, dust/road protection, and no camper/box-door impingement. Prototype one complete direct-profile stainless-bulkhead/metal-plug port with the real HOTTAP hose before buying the second or drilling; retain QD3 only as fallback.
