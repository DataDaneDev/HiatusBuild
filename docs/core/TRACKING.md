...
- Result: preferred cable class is `12 AWG x 2`; at `9.5A`, `78V`, and `20 ft` one-way its screened drop is about `0.733V / 0.94%`. Coil Cable Specialist `1203PU06` is the compact stock lead (`12/3`, `600V`, `25A`, `0.400 in` cable, `1.500 in` coil, `$61.25` shortest version) only after written `>=150VDC`, coiled-ampacity, tangent, wet/outdoor, and cold proof. Autac is the custom `12/2` fallback. Amazon's nearest Iron Forge `12/3` is rejected because the finished product is listed only `125V` with no DC rating. The IMO `SI32-PEL64R-2` remains the fixed two-pole service load-break; one `4S` string presently has no `F-09` string fuse.
- Follow-up: measure routed clamp-center separation at roof down/quarter/half/three-quarter/up; map the front-gap and cab articulation envelope; size factory tangents, drip loop, structural clamps, gland, and drained guard; request written cable data/drawing; then complete full-size force/full-stroke/articulation mockup before purchase or drilling.
