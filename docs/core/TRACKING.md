...
- Orion `48V` input protection is locked as: Lynx `48V+` bus tap -> standalone Mersen `USM1` touch-safe DIN holder -> Orion, with Mersen `ATM20 20A/600VAC/DC` midget fuse and existing `6 AWG` landed directly in the holder's pressure-plate terminals. Purchased FKS/ATO fuses and empty housings are dead-end spare stock, not final install hardware. Do not add pigtails or reducer splices. Lynx Slot 4/`F-05` remains open/blank and is not Orion protection.
