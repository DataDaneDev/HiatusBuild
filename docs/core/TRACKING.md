...
- Impact (1-5): 5
- Likelihood (1-5): 3
- Mitigation: Owner confirms four molded ports on each end—two visibly large and two smaller—with no original obvious/open top port. The new `60 mm` KUS sender opening is now cut, but the `2026-07-27` first `FLS-U` assembly damaged at least one backing-ring thread, left one screw without clamp load, and poured water through the main gasket during an inverted test. Hold tank installation/closure; recover the seized screw and damaged C-ring without enlarging the tank opening, replace the ring/main gasket/matched long screws, inspect the sealing land and sender flange, and pass a static wetting/inverted leak test. Continue to require a frozen end-port map, continuous gravity-fill fall, unrestricted vent rise, compact minimum-fitting pump/accumulator pack, wet/dry separation, leak inspection, and a documented cooler/panel/80/20 removal sequence; do not add manifold/bypass/quick-union complexity by default.
