...
- ID: D-038
- Date: 2026-05-01
- Decision: Supersede the broad `15-series` water-tank exoskeleton plan after purchasing a `35 gal` wheel-well water tank.
- Context: The earlier `15-series` extrusion need was driven by a tall/skinny vertical tank concept. The purchased wheel-well tank is lower, wider, bracketed, and intended to plusnut into bed walls, reducing overturning leverage and removing the main reason for a heavy extrusion cage.
...
- Context: The top-left electrical closet and `3x 48V` batteries spill into the center, the top-right fridge location failed fit, the driver side must support full-time monitor-based work, and the passenger side must absorb the `35 gal` wheel-well tank plus galley plumbing in a compact space.
...
- Result: purchased Progressive Industries EMS-PT30X portable `30A` EMS; Camco TT-30P-to-L5-30R shore cord plus 15A dogbone; Nilight L5-30 inlet; Mollom `6-way` DIN enclosure; ControlGear `30A` breakers qty `2` for AC-in and AC-out main; ControlGear `20A` breakers qty `2`; ELEGRP `20A` GFCI receptacles qty `2` with covers/plates, `10/3` and `12/3` triplex wire, bus bars, ferrule crimper kit, butyl tape, and Sikaflex-221. The original result closed downstream receptacles and row `112`; D-062 later reopened a field-fit downstream-device option, and the `2026-08-07` old-work-box purchase restored row `112` to the active BOM. Row `111` remains inactive historical context, not a current device purchase. The user-provided pop-up multipurpose outlet remains deferred/outside active baseline until details are supplied.
- Follow-up: single-enclosure neutral isolation/PE continuity/no fixed downstream neutral-ground bond still need formal dead-check record before AC-out commissioning. Short AC-in charging validation is now complete; AC-out branch/GFCI commissioning remains separate. Any added downstream devices must use the first GFCI `LOAD` terminals and pass box/cover/clamp/fill/accessibility plus whole-chain GFCI testing; the purchased six-pack does not lock device count.
...
- Result: Active row `171` now owns one `15A` `F-12/F-13-PHVAN` fuse/holder; row `320` is inactive. At the `2026-07-27` decision point, row `170` still planned a high-voltage MEGA for Lynx Slot 3 and row `176` still planned the local `3A` Upfitter #3 enable circuit. Current procurement supersedes that dated state: row `170` is a purchased Victron `200A/80V` five-pack, and row `176` is partially purchased Anyongora holder/mixed-fuse stock pending proof that the received assortment contains an exact `3A` ATO/ATC fuse and suitable holder/wire markings.
...
- Follow-up: owner report `2026-09-02` confirms the MultiPlus is grounded to truck chassis, closing physical installation of that bond. At the next de-energized inspection, verify its `M6 PE` origin, conductor/termination, chassis endpoint, and continuity; separately inventory/install/verify the conductive-shell bond from row `339`; test both GFCIs in inverter and accepted-shore modes; physically confirm Mechman isolated-vs-case-ground behavior; and prove no chassis path bypasses the SmartShunt before alternator commissioning.
...
- Result: The **architecture** decision is a shallow vented cassette field gated by `134 x 66 in` safe supported width; no panel package is released. The tested `4x Lensun 130W + 4x Lensun 70W = 800W / 4S2P` composition remains useful packing evidence, but the generic `70W` retail SKU is not currently verified and hot-start margin is unproven. `4x Solbian SP138 = 552W / 4S1P` fits the modeled `134 x 62 in` skin but Solbian's preliminary `1.25x Voc = 145.5V` exceeds Victron's `145V` operating maximum. Yuma CIGS remains geometry-limited to `400-500W`, with `4S` climate-gated around `-22°C` for the operating limit and `5S` over the controller ceiling at STC. The `760W` AI layouts remain rejected because current Lensun `120W` dimensions differ from the package.
- Follow-up: measure the real common-origin roof/rail/fan/Starlink grid; prove or reject `66 in` carrier width; get Hiatus's all-up interpretation of the `75 lb` limit; then request measured-zone custom/standard module packages. Require current SKU availability, model-specific voltage-temperature data, written mounting/ventilation/warranty approval, exact templates/junction boxes, and final tolerance-aware cold-`Voc`, hot-start `Vmp`, fusing, conductor, disconnect, and moving-jumper release before procurement.

- ID: D-069
- Date: 2026-08-10
- Decision: Freeze the final roof-solar architecture at `6x BougeRV Arch Pro 100W SP003 = 600W`, `3S2P`, on the purchased Victron `150/45`, using four direct-bond modules plus one removable two-module track-side cassette.
- Context: Final measurements corrected the roof to `138 x 63 in`, confirmed the MaxxAir is laterally centered, and identified the `0.625 in` track step. The owner values CIGS durability, crystalline-flex watt density/cost, aluminum heat spreading/removability, and simple buildability, but rejects a full secondary-roof fabrication project.
- Options considered: `400-500W` Yuma CIGS, near-boundary XPLOR `625W`, six Arch Pro `100W`, mixed Arch Pro `800W` plus `250V` MPPT, Lensun `910W` plus broad carrier/`250V` MPPT, full aluminum deck, and rigid modules.
- Decision drivers: honest `1 in` fit tolerances, continuous support across the raised track, existing-controller reuse, cold-Voc margin, complete `75 lb` cap, mainstream availability, serviceability, and avoiding fabrication whose complexity outweighs incremental daily harvest.
...
- Follow-up: verify received SKU/labels/manual, build `1:1` templates including junction boxes/leads/grommets, field-check the outside roof lands/track-step transitions and Starlink/fan service, prove the simplest direct-attachment pattern and any minimal step infill, weigh the complete package, and prove cool start plus hot restart/tracking before bonding or travel. Canonical proof: `docs/studies/SOLAR_configuration_matrix.md`.

- ID: D-071
- Date: 2026-08-12
- Decision: Do not release a generic adhesive hook-and-loop perimeter/center grid as the primary highway attachment for the purchased Renogy flexible panels; preserve removability through a mechanically track-anchored low-profile carrier/cassette unless Renogy gives written approval for an exact removable fastener stack.
- Context: The proposed hook-and-loop grid is attractive because it allows panel replacement and creates a small separation. Current Renogy product guidance for the 175W family instead calls for silicone structural adhesive across the panel backside for mobile use and says the grommets are only for non-mobile applications. VELCRO Brand markets engineered solar fasteners for commercial roofs, and Renogy publishes a user-story example using industrial adhesive Velcro, but neither is a mobile wind-uplift design for these received panels on Hiatus gelcoat. 3M's SJ3550 data state typical laboratory values, emphasize that peel/cleavage is the easy removal mode, and require application-specific evaluation.
- Decision drivers: highway windward-edge peel, adhesive-to-gelcoat and adhesive-to-panel-backsheet compatibility, thermal expansion, water/debris entrapment, theft/partial disengagement inspection, full-footprint support across raised tracks, and controlled replacement without sacrificing the roof skin.
- Result: ordinary hook-and-loop, generic VHB, or Dual Lock is not approved merely by multiplying published static strength by strip area. A few millimeters of engaged fastener thickness is separation, not proven ventilation; a sealed perimeter/grid can compartmentalize water and does not provide an open flow path. The removable lead is a shallow track-anchored carrier supporting the entire panel footprint, with panel-to-carrier attachment following the exact received manual or written Renogy guidance. Direct structural-silicone bonding remains a non-removable candidate only after receipt/manual, roof compatibility, coupon, drainage, thermal, and removal-damage gates pass.
- Follow-up: on receipt, photograph labels and backside, obtain the exact current installation manual, ask Renogy in writing whether a specific industrial hook-and-loop/Dual Lock system is approved for mobile highway use, and template/test the carrier before buying attachment stock.

- Status: Superseded the same day by D-072 after the owner reiterated that all carrier/cassette/rack concepts are outside the accepted complexity envelope.

- ID: D-072
- Date: 2026-08-12
- Decision: Keep the purchased Renogy panels on a direct-to-roof attachment path. Do not propose or develop a carrier, cassette, or roof-rack connection unless the owner explicitly reopens it. Evaluate an industrial solar-capable reclosable fastener as the removable alternative to structural-silicone direct bonding.
- Context: The owner's priority is avoiding dozens of hours of design iteration, fabrication, sourcing, added height/weight, snag exposure, and likely redesign for a secondary task. Panel replacement is the only reason to avoid permanent adhesive. Renogy's official user-story guidance says industrial adhesive Velcro around the perimeter and back should provide spacing and retention for flexible panels. 3M's design selection guide includes flexible solar panels fastened to roofing membrane with strength to resist wind, while VELCRO Brand markets flexible-panel solar fasteners and leaves adhesive selection to the actual roof/panel substrates.
- Decision drivers: simplest install, panel replaceability, low height/mass, no new rack, no fabrication project, no sharp bend or unsupported bridge at the `0.625 in` Yakima-track step, and honest hot-roof/edge-peel behavior.
- Result: Industrial hook-and-loop/Dual Lock is a credible candidate rather than a rejected concept. The candidate is about `1 in` full perimeter plus a center cross/internal strips, with a replaceable windward-edge strip only if the mockup exposes a peel start. The track crossing gets no added structure unless the actual panel mockup shows a sharp crease; then use only a small local spacer/taper. Do not buy generic consumer Velcro. A current `SJ3870` Type 250 / `SJ3871` Type 400 pair is relevant for low-surface-energy substrates and outdoor use, but its current `60C / 140F` long-term rating makes hot-soak testing mandatory; exact panel-backsheet and gelcoat adhesion still control.
- Follow-up: receive and identify the panel backside, measure hot roof/panel temperature, test exact fastener/adhesive coupons on representative gelcoat and panel-compatible material, prove controlled removal and edge-peel resistance, then install one panel and inspect from low-speed shakedown through highway speed before repeating. Structural silicone remains the simple fallback if no removable stack passes.

- ID: D-073
- Date: 2026-08-13
- Decision: Route PV through one continuously installed vertical retractile cord in the camper-to-cab gap, with no exterior quick disconnect or manual roof-motion release. Keep the helix entirely outside and pass only a stationary factory straight lower tail through a fixed-camper compression gland.
...
- ID: D-078
- Date: 2026-08-30
- Decision: Purchase the EVIL ENERGY `10 gal` aluminum diesel tank/feed/vent package and repurpose the LF Bros kit's never-fueled `10 L / 2.64 gal` plastic tank as the removable graywater cassette.
- Context: The heater's measured line is `5 mm OD / 2 mm ID`; the selected Amazon package adapts one `-10AN` tank outlet to the original small LF Bros line and provides a remote `5/16 in` rollover vent. The displaced plastic heater tank is close to the previously targeted `2.5 gal` graywater size, avoiding another vessel purchase.
- Decision drivers: reuse purchased hardware, avoid duplicate caps/oversize fuel line, candidate direct Cerbo sender compatibility pending bench-meter verification, removable graywater service, and minimum permanent plumbing.
- Result: diesel feed is `-10AN -> 1/8 NPT shutoff -> 3/16 in barb -> short LF Bros rubber connector -> original rigid line -> filter -> pump -> heater`; the unused lower outlet stays capped and one upper port receives the remote vent while the other is capped after delivered-hardware verification. Graywater uses the `10 L` tank with positive retention, external venting, cleanout access, and a normally closed `1/2 in` dump leg using the existing clear hose; that hose is not the sink-drain trunk.
- Follow-up: inventory and bench-meter the delivered diesel tank, physically mock its roughly `86 lb` full-load restraint/containment and heater route, then mock the real sink tailpiece/waterless trap/`3/4-1 in` drain into the `10 L` tank. Drill no gray-tank outlet until a gasketed bulkhead has enough flat seal land and internal installation access.
...
- Impact (1-5): 5
- Likelihood (1-5): 3
- Mitigation: Inventory and dry-fit the purchased `10 gal` tank/feed/vent package with the heater, rear storage module, tank restraint/containment, floor penetration, complete exhaust/intake/muffler/termination, fuel line, electrical harness, and warm-air path. Use manufacturer clearances, sealed edge protection, combustion-safe noncombustible shielding with air gap where specified, protected support at short intervals, an upright external rollover vent, and the documented metering-pump orientation: outlet toward the heater and upward about `45 degrees`. Keep every exhaust joint/termination out of both the truck gasoline-tank and camper diesel-tank heat/vapor envelopes. Leak-test, CO-test, heat-soak, and inspect with the truck stationary before road use; have a qualified installer review any unresolved fuel-tank clearance.
...
- Preserve KUS sender access and the documented Galley/tank disassembly path now that the `35 gal` tank and surrounding structure are hard-mounted. Gravity-fill vent nipple measurement is captured (`10 mm` OD main land, `11 mm` OD largest barb); replacement vent tube dry-fit remains open.
- Validate purchased galley fixtures/appliances in the integrated counter/storage modules: real Sarlai sink/soap/faucet cutout and under-counter stack, continuously falling drain into the selected never-fueled LF Bros `10 L / 2.64 gal` gray tank, Duxtop use/stow, and Ninja SP151 manufacturer heat clearances, mechanical appliance cradle, internal rack/basket/pan anti-rattle, and plug/service access.
- Lock the exact sink-tailpiece/waterless-trap/top-inlet interfaces and the low gray-tank bulkhead/valve after physical mockup. Reserve the existing `1/2 in` clear hose for dump service, not the sink drain; sink/faucet remain logged in BOM rows `207-208`.
...
- Final passthrough locations for solar and the purchased diesel-tank/heater path: prove tank location/restraint/containment, filler/shutoff access, remote vent termination, filter/pump placement, and protected `5 mm OD / 2 mm ID` line entry before drilling. Shore inlet hardware is selected; final inlet/cable support details are an install-fit task using on-hand clamps/grommets/strain relief.
