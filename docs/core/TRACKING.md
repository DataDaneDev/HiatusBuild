...
- Result: preferred cable class is `12 AWG x 2`; at `9.5A`, `78V`, and `20 ft` one-way its screened drop is about `0.733V / 0.94%`. The owner estimates about `3 ft` clamp-center span down and `7 ft` up. TPC `60241` establishes the dimensional convention: a `1-5 ft` active helix plus two additional `12 in` tangents is a `3-7 ft` overall assembly. That exact cord would be at hard extension at the estimated full-up span, so it is the durability/geometry reference, not the released buy; target a measured custom black `12/2` assembly with roughly `8.5-9 ft` rated overall reach. A short stationary lower tail may pass through the gland and transition after the interior clamp through exactly two accessible sealed butt splices to fixed `12/2`. Four panels remain `4S1P`: use three series mates and only the two free string-end leads into a compact two-polarity transition box; no four-to-one branch/combiner. LAPP SKINTOP `MS-M-XL` `53112025` is the premium M20 gland lead for a measured `7-13 mm` tail. The IMO `SI32-PEL64R-2` remains the fixed two-pole service load-break; one `4S` string presently has no `F-09` string fuse.
