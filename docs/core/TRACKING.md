...
- Resolve Orion `48V` input fuse mismatch: purchased `30A 58V MIDI` stock is not the install default because the Victron table points to `20A` external battery protection on the `48V` side. Find a physical `20A` fuse/holder rated for the actual `48V` bank maximum, validate whether one correctly rated Lynx-source fuse can replace the separate `F-06`, or get manual/vendor approval before using `30A` stock.
