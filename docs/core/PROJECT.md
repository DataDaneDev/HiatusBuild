...
- All four duplex `120V` receptacles and the associated `120V` work are enclosed and in regular owner-reported use. The owner confirms the MultiPlus is grounded to the truck chassis. Physical box/enclosure and chassis-ground installation are closed; retain explicit verification of the installed MultiPlus `M6 PE` bond, the separate conductive-shell bond, GFCI `LINE/LOAD` and downstream trip behavior, polarity, PE continuity, neutral isolation, and labels.
...
- **Electrical safety closeout:** finish the labeled removable DC switch/service face. The `120V` enclosures, four outlet boxes, and MultiPlus-to-truck-chassis ground are owner-confirmed installed/operational; verify the installed MultiPlus `M6 PE` bond, separate conductive-shell bond, GFCI downstream trip behavior, polarity, PE continuity, and neutral isolation rather than reopening completed installation work.
