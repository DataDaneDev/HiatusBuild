...
- **Orion/fuse identity:** use one final standalone `F-06` on the Orion `48V` input positive lead: Mersen `USM1` touch-safe `10x38 mm` DIN holder plus `ATM20 20A/600VDC` fuse, with the existing `6 AWG` landed directly in the pressure-plate terminals. Do not use the purchased housing-only FKS parts or add pigtails/reducer splices. Identify the physically X-marked Lynx fuse by slot rather than assuming it belongs to Orion: Slot 2 requires `F-03 60A/80V` for the MPPT; Slot 4 remains empty. Retain `F-07 60A/80V` MEGA only on Orion `12V` output and `F-11 100A` ANL only on the `12V` battery positive branch.
