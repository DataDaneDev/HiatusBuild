...
- Electrical system status remains live-proven at the system level, but permanent shore AC-in, AC-out/GFCI, full-bank commissioning, road restraint, and alternator first-charge remain separate gates. Factory pod lighting, Maxxair fan, and initial DC outlets move into the immediate `12V` proof pass; solar and audio remain later workstreams. The propane-only rear-box Joolca HOTTAP V2 core package was purchased `2026-07-26`: one HOTTAP V2 Essentials, one Quick-Release HOTTAP Bracket, and one HOTTAP V2 Mount Cover. The matching Flame King `10 lb` aluminum cylinder and a generic two-bracket/two-strap restraint pack were purchased the same day, but the bracket pack is mockup hardware rather than a verified mobile restraint; the purchased add-on pressure gauge is not the default LP path. The heater remains directly outside-mounted and the cylinder is planned for an approved restraint inside the permanently vented box. The camper-port layer now favors a direct all-metal Joolca/GARDENA-profile plug on a compact stainless bulkhead behind an accessible valve; QD3 is fallback only if the direct profile cannot be made clean and reliable. Receipt, bracket backing, approved cylinder load path/vents, real-hose connector fit, port/backer geometry, hose reach, hot flow, drainability, simple travel caps, and operating clearances remain physical gates.
