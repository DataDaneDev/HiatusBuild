...
- Counter/first-trip state: the three-piece black-walnut installation has a mid-September `2026` target. Full temporary tops are dropped; only a rough removable Galley sheet remains optional. Starlink now has a selected one-panel architecture—rugged shielded Neutrik bulkhead, removable retractile roof jumper, ground-cable alternative, and a four-fastener protective mount on movable extrusion crossbars—but the wall cut, coil retermination, and mount order remain physical-fit/bench-test gated. Ground deployment with the complete OEM cable remains the no-rush fallback.
