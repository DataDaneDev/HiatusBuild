...
- Water state: the interior tank/pump/accumulator/PEX pack is substantially assembled and has passed a first owner-observed pump, pressure, accumulator, and ball-valve function test with no visible leaks. Sink/faucet terminations, gravity-fill hardware, and the rear service bulkheads remain open; repeat the leak test with the compartment completely dry because intentional ball-valve discharge obscured the first inspection. The `2026-07-26` propane shower setup and HOTTAP V2/bracket/cover are purchased. The rear interface remains two compact stainless `1/2 in FNPT` / `3/4 in MGHT` bulkheads with permanently threaded metal Joolca/Melnor-profile tap adapters behind the installed accessible valves. Unavailable Melnor `15409` is now only the visual/profile reference; the HQMPC two-set ASIN `B07FHXLKH5`, RAINPAL bulkheads, and GHT travel caps are acquired for the returnable first metal fit test. QD3 remains premium fallback only. Preserve the supplied `4 m + 1 m` shower assembly and prototype a separate roughly `30 in` cold-feed leader after the delivered connector fit is proven. No box-side water plate, articulating arm, large service hatch, interior electric-heater appliance, branch, or cubby remains active.
