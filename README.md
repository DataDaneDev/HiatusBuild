...
- Water order after the electrical closeout: exact FORIOUS faucet adapters, accumulator-swivel stock, the `2-3/8 in` KUS sender hole saw, and the HOTTAP V2 Essentials core package are purchased. The `2026-07-26` HOTTAP order includes one heater, one Quick-Release HOTTAP Bracket, and one HOTTAP V2 Mount Cover; receipt, inspection, backing, and installed-clearance proof remain open. Freeze the tank port/sender map before cutting, complete the minimum-fitting tank/pump/accumulator/tee pack on the bench/driveway, run a full fill/pressure/leak/function test—including the `12V` pump—then hard-mount the proven tank/wet spine and Galley without burying service access. Two compact camper QD3 female no-spill ports use removable QD3-to-Joolca adapter keys with the supplied `1 m` / `4 m` hoses; no box-side water plate, articulating arm, travel-exposed garden adapter, large service hatch, interior electric-heater appliance, branch, or cubby remains active.
