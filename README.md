...
## Current project posture — 2026-08-30
- Immediate safety task: replace the hanging, tape-covered switch bundle with one narrow removable plywood/ABS service face. Individually insulate and label every DC switch terminal; mount the `120V` receptacle in a listed box with cover, conductor restraint, PE continuity, and physical separation from DC terminals.
- Working utilities: Starlink Standard 4 X now operates from the switched dedicated `12V` conversion branch through a `20A` blade fuse, including the installed fixed-body pass-through and retractile jumper. The fridge and water pump are wired and switched. The three independent DC charging branches are in place: one driver-side `100W` outlet and two passenger-side `65W` outlets, each on its own fuse-panel branch.
- AC branch state: four duplex `120V` receptacles are wired—two devices / four plug positions on the driver branch and two devices / four plug positions on the passenger branch. This closes the device-count decision, not commissioning: verify first-device GFCI `LINE/LOAD`, downstream protection, polarity, PE continuity, box/clamp fill, labels, and trip/reset in inverter and shore modes.
- Solar state: all four Renogy `175W` panels have arrived; one-panel physical test-fit plus measured layout indicates the four-panel package fits with Starlink and MaxxAir while using nearly the complete roof. Direct-roof silicone bonding is waiting on exact approved product/prep/cure and a staged one-panel acceptance: static/heat/water/roof-cycle proof, temporary independent test restraint, private low-speed test, then deliberate staged-speed road acceptance before routine highway use or remaining bonds. The PV retractile cord is ordered and remains received-property/gland/structural-clamp/full-motion gated. Keep one `4S1P` string feeding `12 AWG` two-conductor PV wiring, a two-pole disconnect, and the MPPT. The MPPT battery-output branch is `F-03 60A/80V MEGA` in Lynx Slot 2; the single PV string presently needs no separate `F-09` fuse unless received-label/code review changes that.
- Heater/interior state: the LF Bros diesel heater is the main unresolved systems install and must be laid out with the rear driver-side storage module before either is closed in. The EVIL ENERGY `10 gal` tank/feed/vent package and CaLeQi `60 mm` stainless floor turret are purchased. Turret depth is intentionally longer than the `3/4 in` plywood/Lonseal/bed stack so the metal skirt clears the combustible floor and leaves the connections accessible below; release still requires delivered-part dry fit, actual skirt/weld measurement, underbody obstruction/clearance survey around the truck fuel tank and vehicle lines, fully exterior exhaust/muffler, fuel restraint/containment, and exterior venting. Final wall/ceiling/furniture paneling waits behind heater, drain, and access-dependent commissioning.
- Counter/water state: a mid-September walnut-routing appointment is planned; bring the actual sink and soap dispenser for routing. The LF Bros `10 L / 2.64 gal` plastic tank is selected as the never-fueled removable graywater cassette. Prove the bowl, actual drain tailpiece/waterless trap, `3/4-1 in` continuously falling sink hose, top tank inlet/vent, positive retention, and low dump valve before final cuts; the on-hand `1/2 in` clear hose is dump-line stock, not the sink drain.
- Workstation/finish state: install the rear storage module only after the heater tank/pump/fuel-line envelope is proven. Preserve the roughly `24 in` desk until the actual module, chair, knees, and aisle are mocked; shrink toward `20 in` only if the measured seated/entry test requires it. The monitor/laptop arms need a padded hard-stop travel cradle and positive restraint that unloads the arms without pressing on an LCD. Bench gas struts are selected from actual lid weight and hinge geometry and do not replace a closed latch or mechanical open stop.
- Remaining electrical gates: fit `F-04 200A/80V`, install both WS500 temperature sensors and all alternator-path protection, use the purchased USB-A-to-B cable to read and save the current WS500 profile before changing it, then run the controlled first-charge procedure. Battery matching/restraint, final cable/terminal protection, and legal payload verification remain open before normal road use.
