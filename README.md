...
- `12V` state: the Orion and fuse panel are mounted/wired; the buffer-battery `4 AWG` branch is close to completion. Final topology remains `battery + -> F-11 100A ANL -> SW-12V-BATT -> panel main +` and `battery - -> panel main -`. Orion input still needs final standalone `F-06`: Mersen `USM1` touch-safe DIN holder plus `ATM20 20A/600VDC` midget fuse, with both existing `6 AWG` conductors landed directly in the holder's pressure-plate terminals. The purchased FKS/ATO housings/fuses are not used; Lynx Slot 4 remains empty.
