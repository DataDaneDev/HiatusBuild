...
## Current project posture — 2026-07-26
...
- `12V` state: the Orion and fuse panel are mounted/wired; the buffer-battery `4 AWG` branch is close to completion. Final topology remains `battery + -> F-11 100A ANL -> SW-12V-BATT -> panel main +` and `battery - -> panel main -`. Orion input is simplified to Lynx Slot 4 with one verified `40A` MEGA (`58VDC` minimum under the locked `56.8V` charge ceiling; Victron `CIP138040020 40A/80V` is the replacement fallback) feeding the existing `6 AWG` pair directly; no separate input inline/DIN fuse holder remains. Keep `6 AWG` and `F-07 60A/80V` on the Orion 30A 12V output.
- Water order after the electrical closeout: exact FORIOUS faucet adapters, accumulator-swivel stock, the `2-3/8 in` KUS sender hole saw, and the HOTTAP V2 Essentials core package are purchased. The `2026-07-26` propane shower setup also includes the Flame King `10 lb` aluminum cylinder, CALPOSE gauge, cylinder bracket/strap pack, Safoner hatch for a neat propane-hose pass-through through the rear box, and AWW outdoor shower mat. Receipt and fit-up remain open. Freeze the tank port/sender map before cutting, complete the minimum-fitting tank/pump/accumulator/tee pack on the bench/driveway, run a full fill/pressure/leak/function test—including the `12V` pump—then hard-mount the proven tank/wet spine and Galley without burying service access. The camper-port layer is reopened for a real-hose fit test: lead prototype is two compact stainless `1/2 in FNPT` / `3/4 in MGHT` bulkheads with directly mounted metal Joolca/Melnor-profile tap adapters behind accessible valves. Melnor `15409` is the first metal test because Joolca explicitly documents Melnor QuickConnect compatibility; QD3 and its adapter keys are premium fallback only. Preserve the supplied `4 m + 1 m` shower assembly and prototype a separate roughly `30 in` cold-feed leader after the delivered connector fit is proven. No box-side water plate, articulating arm, large service hatch, interior electric-heater appliance, branch, or cubby remains active.
