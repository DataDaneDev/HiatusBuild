...
- Immediate electrical safety closeout: connect the MultiPlus external `M6` PE/chassis lug to a verified vehicle-chassis bond and add the separate conductive-shell bond, then verify continuous PE, GFCI/polarity/neutral behavior, no arbitrary PE-to-DC-negative jumper, and no fixed downstream neutral-ground bond. Complete the labeled removable DC switch/service face if still open.
