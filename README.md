...
This is the single workspace map and update guide for the Hiatus/F-350 camper build. The repo is now in the **working-systems closeout / charging-heater-interior integration** phase: use it to keep the operational electrical/fresh-water baseline, PE safety closeout, solar/alternator commissioning, heater/diesel installation, Bubinga/sink/graywater work, HOTTAP, and interior storage/restraint aligned without creating parallel notes.
...
## Current project posture — 2026-09-02
- Working baseline: the `1S3P` battery bank has been paralleled for an extended period; batteries and ICECO are restrained; Starlink, fresh-water plumbing, KUS/tank, and the `12V`/`48V`/`120V` systems have seen regular use. All four duplex outlet devices are in enclosures, and the owner reports the existing-camper shakedown is good.
- Immediate electrical safety closeout: the owner confirms the MultiPlus is grounded to the truck chassis. At the next de-energized inspection, verify that installed bond uses the external `M6` PE/chassis lug, suitable conductor/hardware, and a sound chassis endpoint; add/verify the separate conductive-shell bond, then verify continuous PE, GFCI/polarity/neutral behavior, no arbitrary PE-to-DC-negative jumper, and no fixed downstream neutral-ground bond. Complete the labeled removable DC switch/service face if still open.
- Solar: the pass-through, two-pole disconnect, stationary wiring, and MPPT landing are complete; no panels are connected. Remaining work is roof/backsheet preparation, structural-silicone bonding/cure, `4S1P` wiring, moving-cord support/guarding, and staged commissioning.
- Alternator: the source remains disabled/not yet active. Owner-reported remaining work is the protected PH-VAN red lead plus read/save/program of the Wakespeed profile, followed by preflight and controlled first charge.
- Heater/interior: install the LF Bros heater, sealed turret, EVIL ENERGY `10 gal` diesel tank/cradle/fill/vent/fuel route, and rear driver-side storage module as one package. Air fryer, Bench lid, monitor/laptop travel state, and general storage remain open.
- Counter/water: final surfaces are a `48 in` live-edge Bubinga Galley and `47 in` live-edge Bubinga Desk. Route/install the sink at the mid-September appointment, leak-check only the new/altered fresh-water joints, and build the separate vented/restrained graywater system.
- HOTTAP/propane: HOTTAP receipt remains pending. Propane geometry is mocked; complete the appliance/bracket/cylinder integration, clearances, leak test, and first burn after receipt.
- Workstation/finish: the FLEXISPOT Foldex chair is ordered. Validate chair/Desk/aisle/roof-down fit when it arrives, then build the monitor/laptop travel cradles and finish removable panels/storage without trapping service access.
