...
- Water order after the electrical closeout: exact FORIOUS faucet adapters, accumulator-swivel stock, the `2-3/8 in` KUS sender hole saw, and the HOTTAP V2 Essentials core package are purchased. The `2026-07-26` HOTTAP order includes one heater, one Quick-Release HOTTAP Bracket, and one HOTTAP V2 Mount Cover; the matching Flame King `10 lb` aluminum cylinder and a candidate two-bracket/two-strap restraint pack were purchased the same day. Receipt, inspection, cylinder fit, approved structural restraint/ventilation, backing, and installed-clearance proof remain open; the purchased add-on pressure gauge is not the default LP path. Freeze the tank port/sender map before cutting, complete the minimum-fitting tank/pump/accumulator/tee pack on the bench/driveway, run a full fill/pressure/leak/function test—including the `12V` pump—then hard-mount the proven tank/wet spine and Galley without burying service access. The camper-port layer is reopened for a real-hose fit test: lead prototype is two compact stainless `1/2 in FNPT` / `3/4 in MGHT` bulkheads with directly mounted all-metal Joolca/GARDENA-profile tap adapters behind accessible valves; QD3 and its adapter keys are premium fallback only. No box-side water plate, articulating arm, large service hatch, interior electric-heater appliance, branch, or cubby remains active.
