#!/usr/bin/env python3
"""Validate the active and inactive Hiatus BOM CSVs."""
...
DEFAULT_ACTIVE = Path("bom/bom_estimated_items.csv")
DEFAULT_INACTIVE = Path("bom/bom_inactive_items.csv")
CATEGORIES = {
...
ACTIVE_STATUSES = {
    "Purchased", "Partially Purchased", "Included", "Selected", "Planned", "Deferred",
}
INACTIVE_STATUSES = {"Returned", "Retired"}
COST_BASES = {
    "item_subtotal", "purchase_record", "delivered_total", "order_remainder",
    "residual_allocation", "owner_estimate", "planning_estimate", "included",
...
PRIVATE_PATTERNS = {
    "Amazon account-order URL": re.compile(
        r"amazon\.[^\s,]+/(?:gp/your-account/order-details|your-orders/order-details)", re.I
    ),
    "raw order identifier": re.compile(
        r"\border\s*(?:id|number)\s*[:#]\s*[A-Z0-9-]{8,}", re.I
    ),
    ...
    try:
        with path.open(newline="", encoding="utf-8-sig") as handle:
            ...
            if reader.fieldnames != EXPECTED_HEADERS:
                errors.append(f"{path}: headers {reader.fieldnames!r} != {EXPECTED_HEADERS!r}")
            return list(reader)
...
def validate(specs: list[tuple[Path, str, set[str]]]) -> int:
    errors: list[str] = []
    ids: dict[int, str] = {}
    ...
    rows_by_scope: defaultdict[str, list[dict[str, str]]] = defaultdict(list)

    for path, scope, allowed_statuses in specs:
        rows = read_rows(path, errors)
        rows_by_scope[scope].extend(rows)

        for line_no, row in enumerate(rows, start=2):
            row_id = row.get("row", "")
            prefix = f"{path}: line {line_no} / row {row_id or '?'}"
            ...
            if not re.fullmatch(r"[1-9][0-9]*", row_id):
                ...
                if numeric_id in ids:
                    errors.append(
                        f"{prefix}: duplicate row ID; first appears in {ids[numeric_id]}"
                    )
                else:
                    ids[numeric_id] = str(path)
            ...
            if status not in STATUSES:
                ...
                errors.append(
                    f"{prefix}: status {status!r} is not allowed in the {scope} BOM"
                )
            ...
            if not notes:
                ...
                errors.append(
                    f"{prefix}: notes are too terse to identify source/role ({len(notes)} characters)"
                )
            elif len(notes) > 500:
            ...
            if normalized:
                components[normalized].append(f"{scope}:{row_id}")
            for asin in ASIN.findall(notes):
                asins[asin].append(f"{scope}:{row_id}")
            ...
            if cost == "0.00":
                if basis not in {"included", "not_applicable"}:
                    errors.append(
                        f"{prefix}: zero cost requires cost_basis included or not_applicable"
                    )
                ...
                if status == "Retired" and basis != "not_applicable":
                    errors.append(
                        f"{prefix}: Retired zero-cost row requires cost_basis not_applicable"
                    )
            if basis == "owner_estimate" and status not in {
                "Selected", "Planned", "Deferred", "Retired"
            }:
            ...
            if purchase_date:
                ...
                if status not in ACTUAL_DATE_STATUSES:
                    errors.append(
                        f"{prefix}: status {status!r} cannot carry an actual purchase_date"
                    )

    for normalized, row_ids in components.items():
        if len(row_ids) > 1:
            errors.append(
                f"duplicate normalized component {normalized!r}: rows {', '.join(row_ids)}"
            )
    ...
    status_counts = Counter(row["purchase_status"] for row in all_rows)
    scope_summary = " ".join(
        f"{scope}_rows={len(rows_by_scope[scope])} {scope}_total=${totals[scope]:,.2f}"
        for scope in rows_by_scope
    )
    print(
        f"BOM VALIDATION PASSED: {scope_summary} global_ids={len(ids)} "
        f"categories={len(category_counts)} statuses={len(status_counts)}"
    )
    return 0
...
def main() -> int:
    ...
    parser.add_argument(
        "paths",
        nargs="*",
        help="Optional BOM CSV paths. Scope is inferred from 'inactive' in the filename.",
    )
    ...
    if args.paths:
        ...
        for value in args.paths:
            path = Path(value)
            if "inactive" in path.name.lower():
                specs.append((path, "inactive", INACTIVE_STATUSES))
            else:
                specs.append((path, "active", ACTIVE_STATUSES))
    else:
        specs = [
            (DEFAULT_ACTIVE, "active", ACTIVE_STATUSES),
            (DEFAULT_INACTIVE, "inactive", INACTIVE_STATUSES),
        ]
