...
- Follow-up owner correction: the MultiPlus is already grounded to the truck chassis. Physical installation of the MultiPlus-to-chassis ground is closed; the next de-energized inspection should verify its `M6 PE` origin, conductor/termination, chassis endpoint, and continuity. The separate conductive-shell bond plus GFCI/polarity/neutral acceptance checks remain open.
...
- Follow-up order review corrected that vent inventory: the `2026-08-30` package already included the same EVIL `-10AN` female-to-`5/16 in` adapter, `5 ft` of NBR hose, and a remote rollover valve. The later KINTLE kit plus second adapter in row `365` are duplicate-purchase candidates totaling `$25.98`; cancel/return if possible, but preserve purchase cost until a refund is confirmed. Install exactly one selected rollover valve, upright and outside.
...
- Locked the intended heater feed as one lower `-10AN` outlet through the `1/8 NPT` shutoff and `3/16 in` barb into a short cut section of the supplied thick black LF Bros connector hose, then the original `5 mm OD / 2 mm ID` rigid line, filter, metering pump, and heater. The other lower outlet remains capped. One upper port takes the remote rollover vent and the other takes the freed cap after physical verification. Use diesel-rated sealant only on NPT joints; AN connections seal at the flare seat. On receipt, verify whether the filler cap is sealed or has its own rollover-protected vent so it cannot bypass the remote valve.
- Reassigned the LF Bros kit's plastic `10 L / 2.64 gal` tank to removable graywater service; it must remain never-fueled. Use it vented, positively retained, cleanout-accessible, and normally closed. The on-hand `1/2 in` clear hose is reserved for the low dump leg, not the sink drain.
- Sink plumbing remains a physical-interface gate: inventory the actual basket/tailpiece, then mock a compact waterless trap and a continuously falling `3/4-1 in` smooth-wall hose into a top tank inlet. Preferred dump prototype is a gasketed `1/2 in` polypropylene bulkhead on proven flat seal land, `1/2 NPT x 1/2 in` barb, short clear hose, and accessible full-flow inline valve; make no tank hole until the real fitting cutout/washer land and internal access pass.
- Diesel installation remains restrained by the full coupled fit: delivered-hardware inventory, approximately `86 lb` full-tank restraint, exterior or liquid-tight externally vented containment, upright external vent, serviceable fill/shutoff, floor turret, underbody vehicle-clearance survey, exhaust/intake/fuel routing, and rear storage-module geometry. Bench-meter the sender before using a spare Cerbo Tank `DATA/GND` input.
...
- Owner reports fridge and water pump wired/switched; three separately fused DC charging outlets installed (`100W` driver, `2x 65W` passenger); and four AC duplex devices installed as one GFCI plus one downstream duplex on each of two `20A` branches. These are physical progress, not blanket commissioning: exact fuse labels, polarity/load/drop/heat, listed AC boxes/clamps/covers, `LINE/LOAD/PE`, downstream GFCI protection, PE/chassis continuity, and inverter/shore trip tests remain.
- Made the loose/tape-covered switch bundle the immediate safety task. Build one narrow removable plywood/ABS service face, individually insulate and label DC switch terminals, and keep any adjacent `120V` device inside its own listed covered box; do not use finish paneling as the electrical dead front.
- All four Renogy panels are owner-reported received. One-panel physical test-fit plus measured layout indicates coexistence with Starlink and MaxxAir while using nearly the entire roof. Direct structural-silicone bonding waits on exact material/manual compatibility, wash/prep, coupon/cure, drainage, actual weight, and a staged one-panel sequence: static/heat/water/roof-cycle proof, temporary independent track-anchored test restraint, private low-speed shakedown, then deliberate progressive road acceptance before routine highway use or remaining bonds. The PV retractile is owner-reported ordered; exact SKU/rating evidence, final gland size, structural clamps, full `28 in` roof/cab articulation, route drop, and hot MPPT restart/tracking remain gates. The owner plans a compact penetration above Starlink, but `PG9`, M20, or M25 is not locked before cord OD is known.
- The LF Bros diesel heater and rear driver-side storage module must be dry-fitted as one package before drilling. The proposed fuel/exhaust path is near the truck gasoline-tank envelope; heat shield alone is not clearance proof. Freeze complete fuel-tank/pump/line, exhaust/intake/muffler/termination, warm-air, electrical, service-removal, CO, heat-soak, and manufacturer-clearance geometry first.
- The mid-September black-walnut routing appointment requires the actual Sarlai sink and soap dispenser; make no fixture/drain/support cut until the full under-counter stack and continuous drain slope clear the fresh tank. Graywater remains open: first mock a vented, positively retained, normally closed removable vessel around `2.5 gal`; open ground discharge is never the default.
- Preserve the current `24 in x 48 in` Desk until the rear storage module and actual chair are installed/mock-fitted; try a folding/stowed chair before reducing toward `20 in`. Monitor/laptop arms require padded hard-stop cradles and positive travel restraints. The Ninja SP151 needs a mechanical appliance cradle plus internal rack/basket/pan anti-rattle; adhesive tape alone is not crash restraint.
- Alternator closeout is now framed as `F-04 200A/80V`, HEB/KLKD PH-VAN package, both temperature sensors, final cable/support work, and a separate de-energized WS500 read/save/configuration session using the purchased USB A-to-B cable before first field enable.

## 2026-08-25 — PV coil stroke corrected and short-cord screen reopened

- Owner fixed the physical pop-up stroke at `28 in` and re-framed the continuous PV-cord envelope near `24 in` overall roof-down / `52 in` roof-up. This supersedes the active `3-7 ft` endpoint estimate and `8.5-9 ft` procurement target; the interior stationary `12/2` splice can absorb fixed-route length but the moving section itself only needs the pop-up stroke plus working reserve.
- The prior `2-10 ft` stock direction was materially oversized for the current geometry. An idealized `5:1` coil in the current envelope needs about `61.3 in` rated overall reach to satisfy the prior `80%` active-extension screen, so the practical target is roughly `60-66 in`, or up to `72 in` only when natural roof-down length still fits without slack and pull remains low.
- Current stock lead is Coil Cable Specialist `1203PM06-5T`: `12/3`, `600V`, `25A`, black FR polyurethane, `1 ft -> 5 ft`, tangent ends, `0.525 in` cable OD, `1.760 in` nominal coil OD, and `$63.25` at the `2026-08-25` check. Its page does not define active-helix versus complete-assembly length or publish straight-tail dimensions, VDC/wet/cold/force/cycle data, so purchase remains held for written drawing/approval. Its `13.08-13.59 mm` cable tolerance requires the M25 `53112035`, not the `7-13 mm` M20 gland. Compact `1203PU06-5T` and bulkier Nassau `123SOOW1TO5` remain sample/fallback candidates.
- If no short stock cord closes the evidence, send a quantity-one exact-fit RFQ to Philatron and Autac for black `12/2`, approximately `24 in` maximum overall relaxed and `60-66 in` approved working reach, dual axial leads, low spring force, and explicit `>=150VDC`, wet/UV/-40C/continuous-flex approval. No Amazon cord cleared the PV-voltage/environment screen.

## 2026-08-13 — Continuous front-gap solar retractile selected

- Refined the owner-estimated moving span from the misleading stock-label shorthand to about `3 ft` roof-down and `7 ft` roof-up overall. TPC `60241` provides manufacturer evidence that its `1-5 ft` active helix has two additional `12 in` tangents and is therefore `3-7 ft` overall. It is a geometry/durability reference, not the final buy, because estimated full-up would consume all rated extension; seek roughly `8.5-9 ft` rated overall reach after physical measurement.
- Accepted the short-tail approach: pass only stationary intact retractile jacket through the fixed-shell gland, independently clamp it inside, then make exactly two accessible sealed transitions to fixed `12/2`. This shortens stationary retractile cable inside but does not reduce the moving clamp-center travel requirement.
- Resolved the requested four-to-one junction: the array remains `4S1P`, so three panel-to-panel series connections leave only two string-end leads. Use a compact two-polarity weatherproof transition box to the jacketed retractile, not a four-input branch/combiner, which would incorrectly parallel the panels and leave only one-panel voltage.
- Premium gland lead is LAPP SKINTOP `MS-M-XL` M20 `53112025` for a measured `7-13 mm` cord: nickel-plated brass, `12 mm` long thread, IP68/IP69/NEMA 4X-class documentation and body O-ring. If the chosen SOOW cord is roughly `16 mm`, step up to the M25 `53112035`; do not force it through an undersized gland. The on-hand generic glands remain mockup stock.
...
- The separate basic RJ45 continuity-tester purchase is no longer required because the owner established a workable DMM method. Starlink's wall cut remains held for final dish location and a complete inside/outside mockup. The official `NE8FDX-P6-W` rear cutout is one `>=24 mm` center opening plus four `>=3.2 mm` mounting holes on `19.0 +/- 0.1 mm` horizontal by `24.0 +/- 0.1 mm` vertical centers. After scrap proof, use a nominal `1 in` center hole saw and a `#30` bit for the four fastener holes; `15/16 in` and `1/8 in` are nominally undersize for their respective cuts.
- Starlink VHB-backed magnet discs remain pending receipt/template placement of the solar array. Owner reiterated that any solar carrier/cassette or connecting rack is overengineered and outside scope due to fabrication time, sourcing, height, weight, snag exposure, and redesign risk. Corrected the baseline to direct roof attachment. Industrial solar-capable reclosable fastener is a credible removable candidate: Renogy's official user-story guidance supports industrial adhesive Velcro around the perimeter/back, 3M's design guide includes flexible solar-to-roofing-membrane attachment resisting wind, and VELCRO Brand markets flexible-panel solar fastening. Candidate trial is a `1 in` perimeter plus center cross/internal strips, exact substrate-compatible adhesive, no track-step addition unless the panel mockup shows a sharp crease, and one-panel hot/peel/shakedown proof; structural silicone is the simple fallback.

## 2026-08-12 — Renogy 700W panel purchase reconciled; roof/electrical release reopened

- Recorded the owner-confirmed purchase of `4x Renogy 175W flexible monocrystalline panels = 700W`. Checkout evidence was `$649.36` merchandise, `-$32.46` `WELCOME5` discount, `$616.90` net item subtotal, free shipping, `$44.73` estimated tax, and `$661.63` total. BOM row `24` uses the discounted net item subtotal and does not count tax.
- Superseded D-069's unpurchased `6x BougeRV Arch Pro / 600W / 3S2P` product and layout while preserving its measured `138 x 63 in` roof, `75 lb` complete moving-roof cap, separate Starlink/PV routes, and commissioning discipline. Active BOM total moved from `$60,311.61` to `$60,162.53`, a `-$149.08` change from replacing the prior panel estimate.
- Used Renogy's current official `RNG-175DB-H-G2` datasheet as the planning screen because the checkout screenshot does not show the exact suffix. Current `4S` values are `78.0V Vmp`, `95.6V Voc`, `8.98A Imp`, and `9.50A Isc`; published-coefficient cold Voc is `114.86V` at `-40C`. Voltage maximum passes on the purchased Victron `150/45`.
- Reopened controller acceptance for hot operation. An indirect published-coefficient estimate reaches about `61.87V` at `70C`, essentially the Victron `56.8V + 5V` restart threshold before route drop. Received-label verification plus cool-start and hot restart/tracking tests are hard gates; no replacement controller is authorized yet.
- Re-ran roof packing with the measured skin/fan baseline. No all-inside-skin solution was found within the screened constraints. The current candidate fits two rotated panels inside the skin and one direct-mount panel across each side track, using the real outside fiberglass lands plus only minimal flush/tapered infill at the abrupt track step. It is packing evidence only; received junction-box/lead/grommet geometry, `1:1` templates, direct-mount transitions, and Starlink/fan service must pass.
- Panels plus Starlink/TRIO are about `35.2 lb`, leaving `39.8 lb` under the `75 lb` cap for direct-mount materials and every remaining roof component. Added the candidate SVG and synchronized the BOM, canonical solar matrix, system/electrical baselines, PV diagram/fuse schedule, moving-umbilical plan, procurement list, live checklist, tracking decision `D-070`, and weight risk. D-072 subsequently locked direct attachment and excluded carrier/cassette/rack development.
...
- Independently optimized current panel footprints with `1 in` panel/fan gaps, `2 in` Starlink clearance, and `1 in` support margins. Selected `6x BougeRV Arch Pro 100W SP003 = 600W`: four direct-bond panels plus two fully supported on one mirrorable, approximately `91 x 26 in` side cassette. A narrow strip alone is invalid across the `0.625 in` track step; a full secondary roof is unnecessary.
- Locked the existing Victron `150/45` and `3S2P` topology. Each string is `97.2V Vmp`, `113.4V Voc`, and `3.2A Isc`; cold Voc is `142.29V` at `-40F`. A conservative published-coefficient screen estimates hot/tolerance Vmp at `70.91V` at `+85C`, about `9.11V` above the `56.8V + 5V` startup threshold before route drop; measured drop and hot-roof commissioning remain gates. No individual string fuse is presently planned for two parallel strings, subject to received-label/conductor/code verification.
- Panels plus Starlink/TRIO are `38.0 lb`, leaving `37.0 lb` for cassette and all roof hardware under the `75 lb` cap. Product/topology are final. Purchase waits on field survey/templates, cassette design, preliminary weight budget, and any required insurer/Hiatus acceptance; bonding/travel wait on received labels, built-cassette load proof, actual weight, bond coupon/removal proof, final PV route hardware, and hot-roof commissioning.
- PDF/export QA exposed stale diagrams that still reversed D-067's two-shunt order. Corrected the active architecture/overview to `battery negative combine -> Wakespeed shunt -> hard-mounted SmartShunt -> Lynx`, removed the nonexistent SmartShunt-to-Lynx cable, and reduced the no-padding `2/0` black rollup from `35.0 ft` to `33.0 ft` while retaining a short field-fit Wakespeed-to-SmartShunt jumper.
- Updated the canonical solar matrix, electrical/system baselines, Starlink/PV umbilical plan, fuse schedule, live checklist, procurement list, BOM, tracking decision `D-069`, and final layout SVG.
...
- Audited both supplied AI packages, including an independent optimizer rerun. The CIGS package's `400W / 4S` geometry is credible but its electrical recommendation omitted BougeRV's `Voc ±5%`, `-0.28%/°C` coefficient, and Victron's separate `145V` startup/operating maximum; worst-case `4S` reaches that operating boundary around `-22°C`. The physical `500W / 5S` layout exceeds the controller ceiling at STC. Planner-v2's `800W` layouts rely on `0.25 in` edges/gaps, occupy the added fan-louver buffer, and use unsupported generic hot-voltage factors; the `760W` layouts are also invalid against Lensun's current `120W` dimensions.
- Repacked candidate footprints with a `31 x 17 in` MaxxFan body/rear buffer, Starlink, `1 in` outer edge, and `0.5 in` item spacing. `4x Lensun 130W + 4x Lensun 70W = 800W` was infeasible at `134 x 62 in` but feasible at `134 x 66 in`; it is packing evidence only because the generic `70W` retail SKU is not currently verified and the limiting string has only `3.36V` package-assumed hot-start margin. `4x Solbian SP138 = 552W` fits `134 x 62 in`, but its preliminary `1.25x Voc = 145.5V` exceeds Victron's `145V` startup/operating maximum. No current panel package is electrically released.
- Solar remains unapproved for purchase. The durable decision is the architecture: measured shallow vented cassettes if `66 in` is safe, followed by measured-zone custom/standard quotes. Next evidence is one roof/rail/fan/Starlink grid, Hiatus's all-up `75 lb` interpretation, current exact SKU/custom drawings, written mounting/ventilation approval, and tolerance-aware cold-`Voc`/hot-start electrical release.
...
- Owner discussed final interior wood surfaces with the fabricator and is leaning toward three commissioned black walnut pieces: a live-edge Galley countertop, a dimensional-lumber computer desk top, and a dimensional-lumber L-shaped Bench/lid top.
- Captured current Galley target as likely `~4 ft x 19 in` pending exact confirmation, `1.5 in` preferred thickness / `2 in` acceptable if the right slab requires it, with the last `~15 in` of the entry end using an inward-curving live edge if the fabricator can source a suitable slab.
