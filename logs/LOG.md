...
- Follow-on Starlink-plus-solar review supersedes the coil-first interpretation. Keep both systems live in normal road mode through one protected *physical* umbilical, but retain separate electrical circuits. First prototype the continuous factory Standard 4 X cable beside a separately rated continuous-flex PV pair in an energy chain/controlled guard at the sheltered front or highest protected side location; a single fixed-body multi-cable seal may combine the wall entry. Starlink currently discourages modifying or combining its cable, so the L-com `TRD815SZ` coil plus `WPBHC6` remains a non-plug-and-play fallback requiring an adapter/custom termination and full power/heat/reboot/shield validation. A quick-pin upper carrier mount can let both roof ends be safely isolated, capped, and stowed for tight-trail travel. Final PV gauge, disconnect, connector, and combiner choices remain gated on actual panel/string `Voc`, `Isc`, route length, and roof-travel measurements.
