...
- Selected rear wall interface is a thin backed two-port plate using straight-through CPC HFC35 NSF `86600` panel bodies, `83100` hose inserts, and `HFC312L` dust plugs, plus two existing PEX ball valves, two existing `UP120A5` adapters, two `1/2 in FNPT x 1/2 in` barbs, reinforced hose, clamps, and faucet adapters. The HFC35 set is rated to `125 PSI` and `280°F`, replacing the earlier marginal `60 PSI` HFC12 concept.
- HOTTAP stores dry inside the existing bumper box but deploys on feet/stand or a positively locked outboard arm that proves Joolca's `39 in` top, `23.6 in` rear, and `19.7 in` front/side operating clearances. Selected fuel is one Flame King `YSN5LB` `5 lb` QCC1 cylinder. Use Joolca's supplied regulator/hose; no separate regulator or pressure gauge. The rear box is not the default filled-cylinder travel location: prove an accepted upright open-air restraint forward of the camper's rear plane or another approved open cargo location, then deploy the bottle to stable ground. Drain both hoses and heater after every freezing-weather use.
