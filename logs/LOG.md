## 2026-07-25 — Propane-only hot water finalized with two-port rear interface

- Owner closed alternate hot-water branches and selected propane as final. Selected appliance basis is Joolca HOTTAP V2 Essentials: outdoor recreational unit, about `37,500 BTU/hr`, `0.6-1.6 GPM`, recommended `40-60 PSI` / `1.6-3.17 GPM`, and explicitly supported by Joolca for camper-pump input and an existing kitchen faucet. The purchased SHURflo `4008`/SEAFLO pack matches on published values, subject to actual pressure/flow test.
- Permanent water topology is `accumulator -> one 1/2 in PEX tee`; branch to faucet cold, straight to BLUE valve/QD; RED QD/valve through insulated `1/2 in` PEX to faucet hot. At camp, BLUE feeds the HOTTAP. Its red output accepts either the supplied shower hose or a dedicated hot-return hose, one at a time, eliminating an exterior selector valve.
- Selected rear wall interface is a thin backed two-port plate using straight-through CPC HFC12 NSF `63900` panel bodies and `61300` hose inserts, plus two existing PEX ball valves, two existing `UP120A5` adapters, two `1/2 in FNPT x 1/2 in` barbs, reinforced hose, clamps, and faucet adapters. Because HFC12 NSF is rated `60 PSI`, gauge-test the nominal `55 PSI` pump before commissioning.
- HOTTAP stores dry inside the existing bumper box but deploys vertically outside it on feet/removable bracket for operation. Selected fuel is one Flame King `YSN5LB` `5 lb` QCC1 cylinder. Use Joolca's supplied regulator/hose; no separate regulator or tank gauge. Cylinder travels disconnected/upright/valve-closed as portable cargo and sits on stable ground during operation rather than becoming a permanent rear-bumper LP mount. Drain both hoses and heater after every freezing-weather use.
