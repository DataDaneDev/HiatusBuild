...
- Final three-way Starlink-plus-solar review supersedes both the coil-first and chain-first interpretations. Keep both systems live in normal road mode through one protected *physical* front-center umbilical, but retain separate electrical circuits and strain relief. First mock the continuous factory Standard 4 X cable beside a separately rated continuous-flex PV pair as a broad guided S-loop captured by a front guard/pouch; add an `E2.21`-class energy chain only if that loop rubs, kinks, hangs, or packs poorly. Starlink discourages modified/combined cable paths, and the historical L-com `WPBHC6` product path now returns `404`, so a `TRD815SZ`/RJ45 conversion is a bench-test-only last fallback using current Bulgin-class hardware. For trail stow, power down Starlink and open a DC-rated PV load-break before unplugging/capping service connectors. Final PV gauge, OCP, disconnect, connector, and combiner choices remain gated on actual panel/string `Voc`, `Isc`, route length, and roof-travel measurements.
