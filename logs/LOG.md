## 2026-08-12 — Starlink cable terminations completed; solar Velcro route held
...
- Starlink VHB-backed magnet discs remain pending receipt/template placement of the solar array. A generic full-perimeter plus center-grid adhesive Velcro mount for the Renogy panels is not released for road use: Renogy's current product guidance calls for structural silicone on the backside for mobile installations, while available hook-and-loop sources do not provide a Hiatus-specific mobile uplift design. Removability should come from a shallow mechanically track-anchored full-support carrier unless Renogy approves an exact removable system in writing.
...
- Re-ran roof packing with the measured skin/fan baseline. No all-inside-skin solution and no one-side-support solution were found within the screened constraints. The current candidate fits two rotated panels inside the skin and one full panel on continuous support across each side track. It is packing evidence only; received junction-box/lead/grommet geometry, `1:1` templates, both support/load/fairing designs, and Starlink/fan service must pass before fabrication.
- Panels plus Starlink/TRIO are about `35.2 lb`, leaving `39.8 lb` under the `75 lb` cap for both supports and every remaining roof component. Added the candidate SVG and synchronized the BOM, canonical solar matrix, system/electrical baselines, PV diagram/fuse schedule, moving-umbilical plan, procurement list, live checklist, tracking decision `D-070`, and weight risk.
