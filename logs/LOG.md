...
- Starlink route research favors using the existing front truck-bed opening for the fixed cable run, then placing the moving transition on the sheltered front of the popup. Reject a cable that slides through a roof gland; glands seal by clamping stationary cable. First prototype a protected always-connected service loop. If a coil is needed, the verified current family is L-com `TRD815SZ` shielded high-flex Cat5e/TPU coil cord; `TRD815SZ-CH-6-36IN` is the compact `6 in`-to-`36 in` candidate. Its Cat5e data rating is sufficient, but `26 AWG` Starlink power behavior, shield continuity, connector/weather protection, extension/retraction geometry, and repeated popup cycles remain proof gates. Keep weather-exposed quick-disconnect ports out unless the geometry proves a protected permanent loop cannot work.
