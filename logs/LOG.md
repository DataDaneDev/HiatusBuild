...
- Removal-first mounting lead is now TRIO's own four rubber-coated magnets plus four powder-coated `3M 5952 VHB` steel discs for fiberglass (`+$80`). Accept only after full flat contact on sound/smooth/nonporous gelcoat, correct prep, about `72 h` cure, low-speed reinspection, and an independent Yakima-track tether. The prior two-crossbar/Yakima-anchor package remains the mechanical fallback for curved/ridged/flexible/chalky roof contact.
...
- Updated the planning buy list/BOM to about `$430` required core before interior cable/roof attachment/DC branch and about `$555` with the magnetic attachment and normal allowances before shipping/tax. The optional factory-cable-through-panel kit adds about `$27`. Nothing is marked purchased; panel cut, coil termination, roof contact, adhesive bond, and loaded connector testing remain gated.
